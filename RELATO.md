...
_Estado de 22/09 23:1x (placar regerado por `bin/placar_code.sh` as 23:0x, sobre a suite de 7.790 testes que carimbou OK as 22:06; balao e fila de trabalho seguem da lavra de 18/09 -- sao contadores da operacao, nao do Code). Publico: so ids e contagens, nunca nome/CPF, nenhum codigo._
...
<!-- PENDENTES:INICIO -->
## PENDENTES DO RONALD (118) -- aval, "!", corte e smoke esperando voce

_Gerada de `PENDENTES_RONALD.json` por `bin/gerar_pendentes.py` em 22/09 23:06. Entra quando o DRY/pedido nasce, sai quando aplicado. `pendentes` = 118 (aval 14 - corte 66 - smoke 14); `mais_velho_h` = 247 (esperado: nenhum acima de 24 h -- hoje **39 acima**)._

| # | tipo | o que e | desde | idade | afeta | frase para colar |
|---|---|---|---|---|---|---|
| 1 | aval | FABRICADA com escolha declarada (.esteira/furos_estado_veredito, FUROS-ESTADO-PELO-VEREDITO), autorizacao previa de 22/09. Juiz escolhido: CelulaDia.veredito, nao servico_jornada.classificar_falta (esse responde 'qual batida falta em UM dia' por colab/data com banco; aqui a pergunta e o estado do dia de uma grade ja lida). Leituras: (1) 'coberta' -> folga (era None e caia no missing da grade = FURO); (2) veredito que absolve (trabalhou/justificado/coberta) zera os horarios furados do card; veredito que acusa mantem os horarios da grade; (3) 'concorde' continua None (grade furada + cobranca viva = a grade e o fato; absolver foi o erro do B5.3); (4) fato_sem_previsao e fato_em_ausencia continuam fora do mapa (grade responde). O selo antigo test_indices_furados_seguem_vindo_da_grade foi reescrito ('trabalhou' dava [0, 1], agora []). Fatia sem prova local (a corrida nao executa python/docker): o RED/GREEN acontece no rodar.sh. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:435, _TT3, relatorios/furos.py 'def estado_do_dia'). FONTE: codigo VIVO (relatorios/furos.py:24-44, escala/models.py _MAPA_VEREDITO/VEREDITOS_CARTORIO/VEREDITOS_ADMIN). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. NAO MEDIDO quantos dias da auditoria de espelhos mudam em prod (sombra nao consultada nesta corrida). | `` |
| 2 | aval | APLICADO, REVISE: `_F6` declarado em core/juizes.py -> ponto/janelas.py::competencia_fechada, fora de SEM_JUIZ, com a FRONTEIRA _K4 x _F6 escrita ao lado (perguntas diferentes: _K4 'posso recalcular/exportar?' = tranca, por empresa; _F6 'posso ESCREVER ajuste neste dia?' = aprovado/exportado, por colaborador). NAO troquei o juiz da regularizacao externa: a fatia [REG-FECHADO-E-TRANCADO] (22/09 14:01) ja curou aquele sitio com a TRANCA e trouxe 3 selos; trocar para _F6 seria reverter fatia deployada. Consumidores vivos do _F6: registro_batida.py:94 e flip_batida.py:22. | 22/09 00:00 | 23 h |  | `` |
| 3 | corte | Item chamados/views.py#23 (TT7, contador_msgs_mes pelo mes civil) JA TEM FATIA: .esteira/fio_msgs_competencia (FIO-MSGS-COMPETENCIA, em logs/fila_esteira.txt). NAO fabriquei outra (seria duplicata). Ela caiu 17:57 com 'GREEN parcial vermelho' e o vigia (RELATO 18:00) leu como 'vermelho DELA -- nao relanco'. Lido ao vivo em green_parcial.out:469-494: a UNICA falha das 2969 e core.tests.test_haiku_cirurgico.HaikuCirurgicoTests.test_MORDE_contadores_da_trilha (core.intake_conversa.contadores: turnos_por_acao 1.0 != 1.5) -- fora do pacote da fatia (chamados/views.py, core/juizes.py, core/tests/test_contract_juiz_tela.py, selo novo); nada ali toca intake_conversa. NAO provei se o selo haiku fica vermelho na arvore sem a fatia (o teste usa 'dia 21'/'ontem' -- suspeita de relogio solto, nao medida). Enquanto ninguem relancar, o fabricante sorteia este item de novo. | 22/09 18:10 | 4 h | 1 fatia parada; 1 teste alheio vermelho | `relancar fio_msgs_competencia (e olhar test_haiku_cirurgico contadores_da_trilha)` |
| 4 | corte | PAROU: FOLGA-6X1-NAO-MATERIALIZADA: o aval 'FOLGA-6X1 --apply !' chegou, mas o DRY na sombra REPROVA pela guarda do proprio corte ('provar que NENHUM dia trabalhado virou folga'). 48 vinculos 6x1 sem folga cadastrada e com ancora; derivando a folga de ancora+ciclo_dias=7 (delta%7==6) em 8 semanas: 384 dias de folga derivados, 109 furos sumiriam -- e 220 DIAS TRABALHADOS (com batida) virariam folga. Dobro de estrago do que cura. Realidade: so 6 dos 48 tem um dia da semana que bate; 35 NAO TEM NENHUM dia sem batida (a folga deles GIRA) e 7 nao batem. Confirma a lapide de escala/models.py:684-700 ('folga do 6x1 e CADASTRO, nunca ancora; derivar por pivo REPROVADO 3x, 29/08 14,1%'). NAO APLIQUEI: seria escrever folga errada em folha, em 220 dias de gente que trabalhou. | 22/09 00:00 | 23 h |  | `` |
| 5 | corte | TT2 'quais sao os turnos do colaborador (pares de batida)?' em colaboradores/services/situacional.py:120-123 (`'estado_app': ('sem device ativo' if not dev ... 'instalou, nunca bateu' ... 'bateu e parou' ... 'ativo')`, registro core/juizes.py:377-378). NAO FABRIQUEI FATIA porque o proprio registro diz 'deveria ler nenhum declarado': nao ha autoridade para onde trocar a re-derivacao, e declarar juiz e corte seu. Alem disso o sitio NAO responde a pergunta TT2: le ao vivo, ele classifica ENGAJAMENTO DO APP (device ativo? bateu alguma vez na vida -- bateu_vida? tem batida recente no periodo do painel -- batida_recente?), nao pares de batida nem turno; turnos_do_colab/parear_turnos nao cabem aqui. Saidas, todas suas: (1) reclassificar a linha para SEM_JUIZ (pergunta 'o colaborador usa o app?', sem juiz) e tirar de TT2 -- o contador de core/tests/test_contract_juiz_tela.py anda junto; (2) declarar um juiz de 'estado do app' (hoje so existe esta leitura); (3) manter como pendente declarado. Nao toquei core/juizes.py nem o contador. | 22/09 00:00 | 23 h | 1 sitio (registro TT2 estado_app; 0 juizes candidatos) | `corte Ronald: mover estado_app para SEM_JUIZ (ou: declarar juiz / manter)` |
| 6 | corte | F6 'a competencia do dia esta fechada (pode escrever)?' em chamados/services/regularizacao_ext.py:88-95 (`_folha_fechada`, impressao `colaborador=reg.colaborador, ano=_d.year, mes=_d.month).first()`, registro core/juizes.py:306). NAO FABRIQUEI FATIA porque _F6 NAO TEM JUIZ DECLARADO: esta em SEM_JUIZ['feriado/prazo'] (core/juizes.py:206, 'nenhuma leitora: trancar_periodo grava a chave da COMPETENCIA e quatro leitores consultam o mes civil'). Declarar autoridade e corte seu, nao meu. O MOTIVO de nao haver juiz e real, e nao e esquecimento: existem HOJE duas leis vivas e DISCORDANTES de 'fechado barra a escrita' -- (1) ponto/janelas.py:75-90 `competencia_fechada(colaborador, timestamp)`, que se declara 'LEI UNICA de mes fechado', por COLABORADOR, fechado = FechamentoMensal.status in ('aprovado','exportado'), e que desde o seu corte CALENDARIO-UNICO (17/09) le a COMPETENCIA do dia, nunca o mes civil; (2) ponto/services/fechamento.py:465-472 `competencia_trancada(empresa_id, mes, ano)`, juiz JA declarado de _K4 (core/juizes.py:188), por EMPRESA, fechado = PeriodoFechado sem reaberto_em, e cuja docstring diz em letra que aprovada/exportada sao MARCOS, 'nunca fechada'. Escolher uma das duas para _F6 e decisao de negocio que nao esta escrita. O texto de SEM_JUIZ esta DESATUALIZADO (escrito 14/09, antes do CALENDARIO-UNICO de 17/09 que fez competencia_fechada virar leitora de competencia); o comentario core/juizes.py:915 ('competencia_fechada fica no _F6') e intencao, nao declaracao. DESCARTEI a cura barata (o sitio perguntar so `janela_atual` -- juiz declarado de _F5 -- e seguir com o literal de status): depois dela o corpo de `_folha_fechada` vira copia linha a linha de competencia_fechada, a linha de _F6 sairia do registro com o sitio AINDA respondendo _F6 por conta propria, e o pendente irmao de _K4 na MESMA funcao (core/juizes.py:325, 'copia de competencia_fechada') ficaria mais verdadeiro, nao menos -- meia-correcao. EFEITO VISIVEL HOJE, se voce quiser peso para decidir: nos dias >= corte (21-31) o guard le o FechamentoMensal da competencia ANTERIOR; se ela estiver aprovada/exportada, o item e barrado com 'reabra antes' mesmo pertencendo a competencia ABERTA -- e o mesmo bug que o CALENDARIO-UNICO curou para as 241 perguntas de validacao. A reg nao morre: cai no ramo 'fossil invertido' (regularizacao_ext.py:177-182), fica status='confirmada' sem batida e espera `retomar_regularizacoes_confirmadas`. NAO MEDIDO quantas regs estao presas assim -- FONTE juliani.chamados_regularizacaoexterna + chamados_regularizacaoitembatida na sombra / UNIDADE RegularizacaoExterna / UNIVERSO os 3 CNPJs / EXCLUSOES status != 'confirmada' (consulta na sombra nao autorizada nesta corrida). PARA QUEM FOR FABRICAR DEPOIS DO SEU CORTE: se a escolha for competencia_fechada, a assinatura e (colaborador, timestamp) e devolve MENSAGEM ('' = pode escrever), nao status -- o ramo corrigir/remover ja tem `alvo.timestamp` para entregar; o ramo adicionar so tem `item.data` e precisa de ponte data->datetime aware (meio-dia local e seguro), e as duas mensagens de erro dos chamadores (:110 e :150-151) passam a repetir a do juiz. Nao toquei core/juizes.py nem o contador de ponto/tests/test_contract_juiz_feriado.py:95 (22): a linha do registro so sai com corte seu. | 22/09 00:00 | 23 h | 1 sitio (registro F6; 2 leis candidatas; regs presas NAO MEDIDAS) | `corte Ronald: declarar _F6 = ponto/janelas.py::competencia_fechada e tirar de SEM_JUIZ (ou: competencia_trancada / manter)` |
| 7 | corte | F3 'feriado trabalhado paga dobra ou hora simples?' em ponto/views.py:1677 (`hora_extra_100 = request.POST.get('hora_extra_100') == 'on'`, registro core/juizes.py:301). NAO FABRIQUEI FATIA porque o sitio NAO re-deriva a resposta do juiz: ele so COLETA um check do admin. O valor sai do POST, passa pela porta (ponto/services/ausencia.py:72 e :229) e para no campo Ausencia.hora_extra_100 (ponto/models.py:590). LIDO POR NINGUEM -- censo de leitores fechado: grep de hora_extra_100 em todo o app da 4 familias e nenhuma le o campo do MODELO: (a) o campo em si; (b) 11 sitios em ponto/motor_calculo_v2.py, que sao OUTRO objeto (o atributo homonimo do dataclass PeriodoTrabalho, :148, que o motor ESCREVE por conta propria); (c) a escrita da tela/porta; (d) o selo ponto/tests/test_lancar_casca.py:65 test_hora_extra_100_preservado, que pina justamente o campo morto. Por isso trocar a linha por uma pergunta ao juiz declarado (ponto/motor_calculo_v2.py::CICLOS_FERIADO_SIMPLES) nao muda nada: qualquer selo novo ficaria VERDE na arvore original E na curada, e selo que nao morde nao prende nada. Isto nao e contrato 1 (um juiz por pergunta) e sim contrato 3: hoje o parametro nao e CONSUMIDO nem esta ROTULADO 'sem efeito'. As tres saidas, todas suas: (1) CONSUMIR -- motor/export passam a ler Ausencia.hora_extra_100; e zona inviolavel (motor_calculo_v2 + folha) E mudanca de LEI de dinheiro, porque um check do admin passaria a vencer o juiz do feriado (feriado_12x36_em_dobra / CICLOS_FERIADO_SIMPLES); (2) ROTULAR SEM EFEITO -- tirar o checkbox de templates/ponto/lancar_ausencia.html:73-78 e o campo do POST, mantendo a coluna; e front, so sobe com o seu smoke de clique; (3) MANTER como pendente declarado. Detalhe que pesa na (2): o checkbox NAO e gated por tipo -- nada em template ou JS mostra/esconde `campo-hora-extra`, entao ele aparece em TODO lancamento de ausencia, inclusive atestado e ferias, e nao so no tipo 'feriado_folga' (que e vigente: ponto/catalogo/ausencias.py:50, SUPRIME, e vira faixa 'folga' em ponto/precedencia.py:63). Qualquer que seja a saida, o selo test_lancar_casca.py:65 anda junto. NAO MEDIDO: quantas linhas tem o flag ligado em prod -- FONTE juliani.ponto_ausencia na sombra / UNIDADE linha de Ausencia / UNIVERSO os 3 CNPJs / EXCLUSOES nenhuma (a consulta na sombra nao foi autorizada nesta corrida). Nao toquei core/juizes.py nem o contador de core/tests/test_contract_juiz_tela.py: a linha do registro so sai com corte seu. | 22/09 00:00 | 23 h | 1 sitio (registro F3; 0 leitores do campo; linhas com o flag ligado NAO MEDIDAS) | `corte Ronald: rotular hora_extra_100 sem efeito e tirar o checkbox (ou: motor consome / manter)` |
| 8 | corte | A6 'o colaborador esta de ferias hoje?' em ponto/services/triagem_batida.py:53 (`elif colaborador.situacao == 'ferias':`, registro core/juizes.py:281). O ramo NAO dispara: ninguem grava situacao='ferias' em codigo (a porta so grava 'desligado' em colaboradores/services/desligamento.py e 'afastado' em ponto/views.py); so o Django admin (colaboradores/admin.py:23) alcanca o valor. PROPOSTA (default): APAGAR o elif (linhas 53-58) -- nao toca a API, e quem TEM agendamento segue avisado pelo juiz, que ja responde nesta casca (ponto/services/aviso_da_batida.py:73 -> ferias/services.py::em_gozo_hoje). NAO DECIDI porque (a) a casca irma barra ferias PELA situacao (api/views_core.py:378-386, api/views.py:395) e isso foi pinado como LEI em 21/09 -- test_afastado_na_batida.py:110-118 'desligado e ferias barram', com o aviso de que mudar ferias de carona em outra fatia e mudanca de LEI; (b) o proprio sitio esta tagueado 'decisao Ronald' em ponto/tests/test_ferias_dia21.py:17 ('orfaos gemeos'). Apagar = a web deixa de sinalizar quem for marcado no admin sem agendamento (API segue 403). Manter = fica no registro como pendente declarado. A terceira leitura (o elif perguntar ao em_gozo_hoje e virar vivo) esta FORA: o ramo abre chamado acesso_bloqueado por _Chamado.criar com exists() na mao, fora do juiz de duplicado (abrir), e viraria 2o emissor do mesmo fato contra chamados/detector_anomalias.py:113 (contrato 2). NAO MEDIDO: quantos colaboradores tem situacao='ferias' hoje (a consulta na sombra nao foi autorizada nesta corrida) -- FONTE juliani.colaboradores_colaborador na sombra / UNIDADE colaborador / UNIVERSO os 3 CNPJs / EXCLUSOES nenhuma. | 22/09 00:00 | 23 h | 1 sitio (registro A6; 0 colaboradores medidos) | `corte Ronald: apagar o gate de ferias da web (ou: manter)` |
| 9 | corte | CORTE RECEBIDO 12:5x ("C7 leitura 1": o admin segue fechando o silencio; a regra unica vale para o fechamento automatico) -- fatia em montagem; sai daqui quando estiver no ar. C7 'disputa fecha quando': a regra unica (todas as perguntas materializadas ou com via + celula concorde) deixa fechar so 32 das 456 disputas abertas; hoje o admin fecha 396, e 364 delas tem pergunta muda sem via (o corte E1 de 03/09 diz que fechar e o veredito do admin sobre o SILENCIO). (1) o admin segue fechando o silencio e a regra unica vale so para o fechamento automatico; ou (2) a regra unica vale para todos e as 364 deixam de poder ser fechadas pelo admin? | 19/09 09:40 | **85 h** | 456 disputas | `corte Ronald: C7 leitura 1 (ou: leitura 2)` |
| 10 | smoke | GEO-PAINEL: pino de GPS do painel situacional volta a abrir o mapa (front na arvore, fora do git) | 17/09 13:00 | **130 h** | 1 tela | `smoke OK: GEO-PAINEL` |
| 11 | smoke | COBRAR-DIA: botao 'Cobrar este dia' na caixa do Resolver dia | 17/09 10:47 | **132 h** | 1 tela | `smoke OK: COBRAR-DIA` |
| 12 | smoke | SOLIC-FLAG: Solicitacoes no app por empresa (desligado por padrao) | 17/09 11:34 | **131 h** | 1 tela | `smoke OK: SOLIC-FLAG` |
| 13 | smoke | FORM-CATALOGO: 'Nova solicitacao' com 7 areas pelo catalogo | 17/09 12:24 | **130 h** | 1 tela | `smoke OK: FORM-CATALOGO` |
| 14 | smoke | PROPOSTA-EVIDENCIA: faixa da proposta de escala no fio com a evidencia dos 28 dias | 17/09 17:54 | **125 h** | 1 tela | `smoke OK: PROPOSTA-EVIDENCIA` |
| 15 | smoke | lista ESPERA SMOKE do TICKETS (front fora do git): ORDEM-10 UI-3/4/5, ORDEM-12 TELA-1730, ORDEM-15 R15-TOKEN, GATE-FERIAS-AVISO, SW-CASCA-VERSAO, PROPOSTA-NO-FIO, ATESTADO-POR-MINUTOS, E4, E7, ESPELHO-COLAB-APP-PENDENTE, BUG-140-BOTAO-OCULTO, ESPELHO-COLAB-PENDENTE, ORDEM-20 SUPORTE-HASNER, ORDEM-21 ROTULO-SOLICITACAO, ABA-CHAMADOS-DO-COLAB | 12/09 15:30 | **247 h** | 15 telas | `smoke OK: <nome da fatia> (uma por vez)` |
| 16 | smoke | WIZARD-12x36-FASE refeita (2o smoke): Colaboradores > Vincular > um 12x36 (col99) -- calendario no painel, dentro de 'Vincular escala', abaixo da escala, 7 colunas, legenda em 1 linha, 'Inicio do turno dd/mm -- marcado pela paridade real (N% dos dias)', 'N furos antes -> M depois', um botao so 'Salvar'; lista de Vincular intacta; componente do ciclo no wizard (folgas, 12x36) + REGRA 3 (vigencia da fase nova; Inicio da apuracao: a partir da mudanca x desde o vinculo; caso col901 mat 1757 -> 18/09) | 18/09 14:26 | **104 h** | 1 tela + 1 Pauta | `smoke OK: WIZARD-12x36-FASE` |
| 17 | smoke | TELA-FOLGA-ROTULO: no calendario do colab, o Ok verde so aparece em dia de trabalho com o marco batido; folga, feriado e ausencia nunca ganham Ok (front na arvore, fora do git) | 18/09 17:16 | **101 h** | 1 tela | `smoke OK: TELA-FOLGA-ROTULO` |
| 18 | smoke | AUSENCIA-TIPO-REJULGA-TELA: painel da ausencia de tipo que nao pede documento diz 'Documento: nao exigido para este tipo' no lugar do aviso vermelho (front na arvore, fora do git) | 18/09 19:31 | **99 h** | 1 tela | `smoke OK: AUSENCIA-TIPO-REJULGA-TELA` |
| 19 | smoke | PAUTA-DO-DIA: 'Abrir Pauta DP' no dia de competencia fechada cria a pauta colada no dia (o dia no corpo); a ficha do colab lista as pautas dele (front na arvore, fora do git) | 18/09 19:42 | **99 h** | 1 tela | `smoke OK: PAUTA-DO-DIA` |
| 20 | smoke | CADASTRO-X-REALIDADE: Plano de Escalas > 'Cadastro x realidade' (no lugar de 'Escalas propostas (LIMBO)') -- lista por posto, vinculo, assinatura e botao da acao; coluna destino sugerido; contador 300 de 533; botao PDF padrao com 'o que este PDF contem' ao lado e o PDF da mesma lista (front na arvore, fora do git) | 18/09 20:56 | **98 h** | 1 tela + 1 PDF | `smoke OK: CADASTRO-X-REALIDADE` |
| 21 | smoke | FILA-VALIDAR-EM-LOTE (na esteira 14:25; smoke quando o relato disser NO AR): Central > 'Validar em lote os N coerentes' (so classe A, hoje 35) > conferir a previa no drawer > validar; e no copiloto 'quantos posso validar em lote?' tem de responder o numero e apontar o botao. | 19/09 14:25 | **80 h** | 1 tela + copiloto | `smoke OK: FILA-VALIDAR-EM-LOTE` |
| 22 | smoke | LEMBRETE-EM-LOTE (na esteira 14:38, atras do lote de validar; smoke quando o relato disser NO AR): Central > fila Cobrar > 'Lembrar quem tem pergunta no app (N colabs)' > conferir a previa > enviar (um push por colab, 1/dia; sem push vira Pauta de posto); e no copiloto 'quantos tem questionario pendente no app e posso lembrar?' responde o numero e aponta o botao. | 19/09 14:38 | **80 h** | 1 tela + copiloto | `smoke OK: LEMBRETE-EM-LOTE` |
| 23 | corte | Tela para segunda: RECUSA-COM-PORTA, UI-TOOLTIP-FILA e WIZARD-12x36-FDS nao tem definicao em ticket nem no relato -- o que e cada uma? | 19/09 15:05 | **80 h** | 3 fatias | `(uma linha por fatia com o que ela faz)` |
| 24 | aval | Passivo da ata: re-lavra pelo cartorio (316 colabs, push desligado) derruba o contador principal de 28 para 9, mas emite 49 cobrancas e 920 protestos. Os 120 'nunca bateu em dia coberto' e os 294 'sem celula com vinculo' nao se curam por re-lavra (precedencia e gerar_celulas). Recomendado nao aplicar. | 19/09 16:20 | **78 h** | 316 colabs / 19 dias | `aval Ronald: re-lavra ATA (ou: nao)` |
| 25 | corte | Suite rapida: depois do hash MD5 so no CI (-27%), o maior peso que sobra e UM teste -- test_grade_fuzz.test_fuzz_ancorado, 162 s, que nao se divide entre os 4 processos (a suite nao fecha em 4 min com ele). Duas saidas, as duas mudam o que ele exercita: (a) re-semear por forma (menos sorteios, mesma cobertura declarada); (b) montar a grade em memoria (mais rapido, mas deixa de exercitar o caminho do banco). Ou (c) deixar como esta e aceitar ~6 min. | 20/09 12:40 | **58 h** | 162 s de 486 s | `corte Ronald: fuzz da grade (a) re-semear / (b) em memoria / (c) fica como esta` |
| 26 | corte | EXPIRA_EM NA VIRADA (achado 21/09 00:3x): a graca de -1 dia de `_dentro_da_janela_viva` vale a CADA competencia, entao o dia 20 (ultimo da competencia) expira um mes depois do dia 19 -- medido em prod: 18 e 19/09 -> 21/10; 20 e 21/09 -> 21/11. A graca e do TURNO que cruza a meia-noite (e ai vale uma vez so, na borda de entrada) ou do EVENTO (e ai o selo pede demais e o texto dele muda)? Selo `test_selo_lavrado_orfaos::test_expira_em_devolve_FIM_DE_COMPETENCIA_nao_prazo_fixo` em quarentena com este motivo, 48 h. | 21/09 00:35 | **46 h** | todo evento do dia 20 de qualquer competencia | `corte Ronald: graca de -1d e do turno (ou: do evento)` |
| 27 | aval | COL650, licenca-maternidade de 4 meses que a lei nao enxerga (achado 21/09 01:5x): AUS#3186 aprovada desde 13/05/2026 SEM data final e com dias_corridos=1. `ausencia_cobre`=None, `fatos_do_dia`='trabalho', 61 celulas `nunca_bateu`, 24 chamados abertos contra ela, 08/2026 fechado APROVADO com 0,00 h. (1) Pauta DP para o cadastro por a data de fim (120 dias de 13/05 = 09/09); (2) depois disso, retratar os 24 chamados pela porta e re-lavrar as celulas da competencia aberta; (3) 08/2026 esta aprovada -- retificar ou nao e decisao sua. Contador novo `ausencia_bloqueante_sem_fim` = 1 no placar (esperado 0). Nada executado. | 21/09 01:55 | **45 h** | 1 colab, 24 chamados, 61 celulas, 1 competencia paga | `! col650: cadastro -> retratar -> (retificar 08 ou nao)` |
| 28 | corte | ESCRITOR DE `situacao='afastado'` (ultimo pendente da familia ausencia, 21/09): `ponto/views.py` grava o campo quando o DP lanca a ausencia e NINGUEM reverte quando ela acaba. Parar de gravar mexe em 4 universos que nao sao desta familia (`situacao__in=('ativo','afastado',...)` em inteligencia, relatorios e ponto). E a trilha nao tem NENHUMA linha com 'situacao' em prod -- ou o escritor nunca rodou, ou grava sem trilha, e isso precisa ser sabido antes. (1) o campo continua sendo cadastro e ganha REVERSAO (quem reverte: cron, como o de ferias); ou (2) o campo sai e o juiz `afastado_hoje` responde sozinho, com os 4 universos adaptados. RE-TRIADO 22/09 pelo fabricante de fatia (item `ponto/views.py#3` do registro): NAO fabricado, e a leitura (2) nao e mecanica -- `ponto/services/afastado_avisa.py:26-33` usa o campo de proposito como DIVERGENCIA declarada (cadastro diz afastado e o juiz nao: 11 concordam, 1 so no cadastro, 0 so no juiz), entao tirar o escritor CALA justamente o caso que mais precisa da Pauta. `ponto/views.py:1706-1708` e `TICKETS.md` A-AFASTADO-TRIO ja dizem "espera corte". RE-TRIADO OUTRA VEZ 22/09 (item `ponto/views.py#2` do registro, MESMA ancora do #3): o sitio nao re-deriva a pergunta -- ele ESCREVE um campo de cadastro, e o juiz `afastado_hoje` (ponto/turnos.py:989) ja responde pela Ausencia; nao ha cura mecanica, so as duas leituras acima. Enquanto a linha ficar em `PENDENTES['ausencia/ferias']` sem corte, o fabricante vai sortear este sitio toda vez que a trava A cair abaixo de 3 -- e' o unico item da familia sem cura mecanica. So o corte (ou tirar a linha do registro, que e' palavra sua) para o laco. | 21/09 01:10 | **45 h** | 12 colabs com o campo; 4 universos | `corte Ronald: situacao afastado com reversao (ou: sai e fica so o juiz)` |
| 29 | corte | K8-FM-DE-HOJE x JANELA DE FECHAMENTO (21/09): a fatia que tira o mes CIVIL de 11 sitios do FechamentoMensal e raia DINHEIRO, e a janela barra dinheiro ate o export do DP -- entao a fatia que protege o export esta parada pela regra que protege o export. Medido hoje: `janela_atual` = 21/09-20/10 nas 3 empresas, `hoje.month` = 9, e `FechamentoMensal` mes=9/2026 tem 592 linhas (todas abertas) = a competencia em exportacao. A tela sempre manda `mes`/`ano`, entao o caminho de todo dia nao cai no default; quem cai e POST sem o campo. E `recalcular_fechamento_mes` grava sobre `aprovado` e nao consulta `PeriodoFechado` (K4). (1) abrir excecao da janela para a K8 antes do export; ou (2) ela espera o export como as outras de dinheiro. CONFERIDO 21/09 03:4x: o portao de dinheiro e' so HORA + carimbo da sombra (`sombra.sh --conferir` ja devolve rc=0 com o carimbo de 20/09) e a faixa 03:00-04:45 abre as 04:45 -- a k_fmhoje teria pousado SOZINHA por volta das 05:00 se eu nao a tivesse tirado a mao da fila de integracao. "Nenhuma fatia de dinheiro ate o export" e POLITICA do CLAUDE.md e NAO esta em trava nenhuma: falta um portao de politica (arquivo tipo `esteira.dinheiro_fechado` lido pelo `portoes_abertos`, como ele ja le `esteira.pausada`). Sem ele, a proxima fatia de dinheiro entra sozinha. | 21/09 02:15 | **44 h** | 11 sitios; 592 fechamentos abertos | `! K8 entra antes do export (ou: espera)` |
| 30 | corte | FERIAS na porta da batida (metade que sobrou do corte de 20/09): a metade do AFASTADO esta resolvida e no ar (A-AFASTADO-AVISA e A-AFASTADO-TRIO: afastado nunca bloqueia, so avisa e cobra a supervisao; a porta pergunta ao juiz). Falta a de FERIAS: hoje a porta barra por `situacao == 'ferias'`, campo que NINGUEM escreve, e 15 pessoas que o juiz diz estarem de ferias batem ponto sem aviso nenhum. Pelo juiz elas passariam a ser barradas -- e isso conflita com 'batida de chao nunca e barrada'. Mesmo desenho do afastado (nunca barra, avisa e cobra), ou ferias barra mesmo? | 19/09 16:20 | **78 h** | 15 colabs de ferias pelo juiz | `corte Ronald: ferias nunca barra (ou: barra)` |
| 31 | corte | DIA DO CHAMADO por um juiz so (os 2 ultimos do registro_chamado): trocar a leitura crua do `contexto_json` pelo juiz `data_do_chamado` muda o dia de 4.705 chamados dos 22.765 (20,7%) em DOIS escritores de prod (supra_juiz e cartorio), dentro da janela de fechamento. Nos exemplos o campo cru esta vazio e o juiz acha data. Medido 20/09 19:2x, so leitura. E o que falta para registro_chamado = 0. | 20/09 19:20 | **51 h** | 4.705 chamados, 2 escritores | `corte Ronald: dia do chamado pelo juiz (com DIFF antes)` |
| 32 | aval | SLA do chamado pelo juiz: hoje escala por idade >= 20 min cravada; o juiz le o prazo gravado e 'sem prazo = nao estourou'. Trocar direto CALA 27 escalonamentos vivos (230 disputas, 203 concordam, 27 so pela idade, 0 so pelo juiz; 983 dos 2.535 vivos sem prazo). Proposta em dois passos: 1o o emissor carimba o prazo no nascimento, 2o o leitor troca. | 20/09 19:20 | **51 h** | 27 escalonamentos, 983 sem prazo | `aval Ronald: carimbar prazo no nascimento, depois trocar o leitor` |
| 33 | corte | PAUTA-HUMANA, a pergunta que decide a fatia 1 (censo em app/docs/PAUTA-HUMANA.md, 21/09 03:0x): os tres campos (titulo/linha/acao) sao COLUNAS do modelo -- selo forte, Pauta sem acao nao nasce, mas migration e as 500 vivas nascem sem eles -- ou DIVISAO na hora de mostrar (zero migration, mas o selo passa a afirmar sobre heuristica)? Recomendacao: colunas OPCIONAIS na fatia 1, selo exigindo so para Pauta nova, passivo vira contador que so encolhe. Medido: 500 vivas, 88% com caixa-alta tecnica, ZERO comecando com verbo; telefone preenchido em 778 de 859 (a porta do 'Ligar' cobre 91%); o valor da Pauta de pagamento NAO tem campo. Segunda pergunta: as 41 vivas de 'ti' (aviso de esteira) ficam na fila do humano ou saem dela? | 21/09 03:10 | **43 h** | 500 pautas vivas, 238 do esmeril | `corte Ronald: pauta humana em colunas opcionais (ou: divisao na hora de mostrar)` |
| 34 | aval | INTEGRADOR SEM QUEM O LIGUE (achado 21/09 07:4x): o `hasner-integrador-off.timer` desliga E DESABILITA o `hasner-integrador.timer` as 06:00 (aval seu de 20/09 17:3x, 'so a noite ate o resize') -- e nao existe timer para LIGAR de volta. `is-enabled` responde `disabled` agora. Consequencia: hoje a noite nada sobe sozinho; a fila fica cheia e quieta ate alguem rodar `systemctl --user enable --now hasner-integrador.timer` na mao. Mesma familia dos crons declarados e nunca instalados (04/09). Cura de uma linha: um `hasner-integrador-on.timer` as 22:00, irmao do que desliga. Nao liguei: o aval diz 'desligado de dia' e producao esta acordada. | 21/09 07:45 | **39 h** | 2 fatias PRONTAS na fila (AUS-COLISAO, I-RAIA-VAZIA) | `aval Ronald: criar o integrador-on as 22:00 (ou: ligo na mao toda noite)` |
| 35 | corte | APROVAR/TRANCAR A COMPETENCIA NAO TEM JUIZ (censo da familia fechamento, 21/09 07:5x, em app/docs/FAMILIA-FECHAMENTO.md): a pergunta 'a competencia pode ser aprovada/trancada, e o que bloqueia' tem SEIS sitios e NENHUM juiz declarado -- cinco deles de DINHEIRO, cada um com um crivo diferente: o lote cego pula conta vazia mas a selecao manual, o aprovar_colaborador e o TXT aprovam assim mesmo; 12x36 sem ancora fica fora do lote; aprovar_colaborador olha so turno aberto; a tranca so acontece com empresa e sem pulados (sem empresa aprova todas e nunca tranca); e o TXT aprova pelo `entra` de classificar_export, sem os crivos do botao e sem trancar. Com 592 fechamentos de 09/2026 abertos, e por aqui que eles passam. Qual e o juiz unico de 'pode aprovar' e de 'pode trancar', e o que cada um barra? | 21/09 07:55 | **39 h** | 6 sitios (5 de dinheiro), 592 fechamentos abertos | `corte Ronald: juiz unico de aprovar/trancar (e o que ele barra)` |
| 36 | aval | COL935: bateu 05 e 06/09 e a admissao/vinculo dela comeca em 07/09 (medido 21/09 08:0x). Nos dois dias ha batida e NENHUMA celula, porque nenhum vinculo cobre o dia -- o cartao conta pela batida e o TXT le a celula, e sai a divergencia de adicional noturno de 9,11 h (cartao 72,05 x TXT 62,94) do gate item 3. Varri os 7 divergentes: ela e a UNICA com batida em dia sem celula; os outros seis tem as 31 celulas e a causa e outra. Cura sem codigo: o DP corrige a data de admissao/inicio do vinculo para 05/09 e o TXT dela sai certo NESTA competencia. Se a data de 07/09 estiver certa, entao as batidas de 05 e 06/09 e que precisam de decisao. | 21/09 08:10 | **38 h** | 1 colab, 9,11 h de adicional noturno, competencia em exportacao | `! col935: admissao/vinculo em 05/09 (ou: as batidas de 05-06/09 nao valem)` |
| 37 | corte | CONTRATO TROCA-DE-PLANTAO (topico de 21/09, DINHEIRO, 09 ainda nao exportada). Medido: 8 trocas na 09, todas emp 2, lancadas como TipoAusencia('troca_de_plantao', efeito='abona') que NAO existe no codigo; o par cedido<->assumido so vive no texto livre da descricao. O dia ASSUMIDO e folga na celula de quem assume, entao as horas caem em horas_folga_trabalhada -> rubrica 200 (HE 100%) e o adicional noturno e a intrajornada do dia nao saem: 33,22 h na 200 que deviam ser jornada normal (col841 3,93 / col276 12,06 / col190 12,14 / col852 5,09). 2 RED fora da arvore vermelhos pelo motivo certo (red_fora_da_arvore/test_troca_de_plantao.py) e ensaio na sombra com o delta por colab. O CONTRATO a cortar: (1) troca vira PAR declarado (dia cedido <-> dia assumido) na porta de ausencia/escala; (2) a celula do dia assumido vira trabalha=True pelo gerador; (3) precedencia troca > folga; (4) dia assumido paga jornada NORMAL + AN + intra, SEM HE 100; dia cedido sem desconto e sem furo; (5) troca sem par acende contador `trocas_sem_par`. Nada construido. OrdemSubstituicao e AgendaDia ja existem e nenhuma das duas alcanca a folha. | 21/09 10:25 | **36 h** | 8 trocas na 09 / 33,22 h na rubrica 200 | `corte Ronald: contrato TROCA-DE-PLANTAO` |
| 38 | smoke | POPOVER-RESOLVER-DIA pronto e FORA DO GIT ate o seu smoke (FRONT SEM SMOKE NAO SOBE, BUG 73). Toca `static/js/popover-acao.js` (novo), `templates/ponto/partials/_veredito_dia.html`, `base.html` e `base_app.html` -- template BASE nas DUAS cascas. Selo `core/tests/test_selo_popover_acao.py` 6/6 verde, com chromium medindo 1366 e 1024. Smoke pedido: abrir o calendario de um colaborador, clicar "Resolver dia" num SABADO ou DOMINGO (ultima coluna) e conferir que os botoes aparecem inteiros e clicaveis; e o smoke de sempre nas duas cascas (colab bate ponto; admin faz uma acao com modal). | 21/09 11:00 | **36 h** | 1366: estourava 50 px; 1024: 99 px | `smoke Ronald: Resolver dia na ultima coluna (SAB/DOM), 1366 e 1024` |
| 39 | corte-dado | CORTE RECEBIDO 21/09 13:5x: **o contrato HORARIO-MOVEL vale**. Contrato de entrada escrito na hora (CLAUDE.md secao 6, 4 linhas) -- FONTE: a 1a entrada de cada dia por `ponto/turnos.py::turnos_do_colab` (nunca Batida crua) contra o marco `hi` de `EscalaColaborador.marcos_do_dia`; UNIDADE: colab-dia, agregado por colab-competencia; UNIVERSO: vinculos ativos 6x1/5x2 na competencia medida; EXCLUSOES: 12x36/24x48 (fase por ancora, nao tem `hi` fixo), intermitente (ja e posicional por desenho), isento de ponto, e os dias sem previsao. PENDENTE de MEDIDA (fork em voo): o limiar '>=90 min em >=30% dos dias' e a separacao dancante x bimodal x deslocamento-fixo so viram numero quando o fork devolver. **O item 3 do despacho (lote seguro = 10 min) NAO esta destravado por este corte**: `tolerancia_marcacao_min` esta em `core/regua_cct.py:44-52 SEM_EFEITO_NO_CALCULO` (o casamento batida x marco nao a consulta), entao usar '10 min da regua' exige LIGAR um parametro hoje decorativo -- fatia de DINHEIRO propria, com DIFF. | 21/09 13:50 | **33 h** | contrato escrito; limiar espera a medida do fork | `corte Ronald: contrato HORARIO-MOVEL vale (recebido 21/09 13:5x)` |
| 40 | corte-dado | CORTE 21/09 16:3x: **guarda da janela AGORA (com DIFF); dia do turno por proximidade so DEPOIS do export**. Estado: fatia 1 construida e GUARDADA fora da arvore (`.fatias_construidas/TURNO-NATIMORTO-fatia1.patch`, `git apply --check` OK) + 2 selos em `red_fora_da_arvore/`. Ela NAO esta no ar -- rodou 27 min em prod por engano (12:13-12:40) e foi retirada. **DIFF ja medido na sombra: NAO e zero** -- 172 batidas trocam de tipo, 188 mudam de sequencia, 185 turnos fecham, 8 colabs movem dinheiro, resto = ZERO confirmado; no TXT so col297 muda de lado (os outros 7 seguem retidos por furo_espelho). **FALTA para a fatia 1 ficar COMPLETA** (corte de 16:1x): o gemeo PERSISTIDO `ponto/nucleo.py:31 dt_fim_previsto_de` (grava no banco a janela nascida da DATA) e os 4 consumidores -- `ponto/selecao_periodo.py:208`, `core/services/painel_op.py:129`, `processar_alertas_turno.py:98,108`, `processar_alertas_avancados.py:69-70`; selo por FUNCAO, nao por arquivo (o de hoje guarda so `ponto/turnos.py`, e `janela_turno_de` tem 6 chamadores); e o DIFF refeito depois disso. | 21/09 16:30 | **30 h** | 172 batidas / 185 turnos / 8 colabs; falta o gemeo persistido + 4 consumidores | `corte Ronald: TURNO-NATIMORTO guarda da janela agora (com DIFF); proximidade depois do export` |
| 41 | corte-dado | CORTE 21/09 16:3x: **o contrato TROCA-DE-PLANTAO vale** (FONTE `eventos_do_fechamento` sobre o turno do dia assumido; UNIDADE colab-dia; UNIVERSO pares de troca na competencia; EXCLUSOES folga trabalhada sem troca, isento, cadastro errado; ESCALA lote + contador `trocas_sem_par`). Medido: 8 trocas na 09, todas emp2, lancadas como `TipoAusencia('troca_de_plantao', efeito='abona')` que NAO existe no codigo; o par so vive no texto livre. **33,22 h na rubrica 200** que deviam ser jornada normal. Os 2 RED estao em `red_fora_da_arvore/test_troca_de_plantao.py`, vermelhos pelo motivo certo. **Pendente de medida**: explicar o +24,14 h do col190 (o ensaio devolveu mais que o dia isolado) e corrigir a Pauta P562 se o numero mudar. | 21/09 16:30 | **30 h** | 8 trocas / 33,22 h na 200 / 2 RED prontos | `corte Ronald: contrato TROCA-DE-PLANTAO vale` |
| 42 | corte-dado | CORTE FINAL 21/09 17:0x: **JSP (emp3) = CLT** (hora noturna reduzida no 12x36), decisao da empresa; **TODAS as demais empresas = a CCT vigente da praca**. **UNIDADE DA REGUA = empresa x praca** -- a CATEGORIA SAI da unidade, e a medicao 'emp2 Londrina com hora reduzida ligada' esta CANCELADA. **Consequencia boa, e ela destrava a fatia A**: a barreira que eu tinha levantado era a categoria nao existir como dado (`Colaborador.cargo` e texto livre, 53% vazio, 32 grafias; em Londrina 138 de 319 sem cargo). Sem categoria na unidade, **isso deixa de bloquear** -- empresa e praca sao dados solidos. Fatia A segue: DIFF so nos **36** da JSP (emp3 Londrina 12x36 noturno), **ZERO** nos 59 da emp2 e nos 7 da emp4, e PARA no "!". `colabs_sem_regua_declarada` nasce em 241 (dono cadastro) e nao bloqueia. | 21/09 16:30 | **30 h** | 36 colabs movem; unidade sem categoria destrava a fatia A | `corte Ronald: REGUA-NOMEADA -- JSP pela CLT, demais pela CCT da praca; unidade empresa x praca (sem categoria)` |
| 43 | corte | GEOFENCE-API (BO Fernando iOS, passo 6) -- MEDIDO 21/09 16:5x. **Nao existe endpoint**: `/api/me/` devolve so o NOME do posto (`api/views_core.py:328`); o app so recebe o VEREDITO no ping-geo, nunca a cerca. **Regra paralela JA EXISTE**: 4 sitios de distancia, e o haversine inline de `api/views_core.py:963-970` (== `api/views.py:2126-2133`) NAO chama o juiz `ponto/services/geofence.py:34-40`. **Fonte da verdade = `Batida.posto_para_calculo` (`ponto/models.py:88-90`), NAO `EscalaColaborador.posto`** -- montar pelo vinculo entrega cerca errada a **49 colaboradores**. Medido: `postos_sem_coordenada` **10 de 205** (Londrina 4, +1 raio de 50 km); colabs multi-posto **0 de 547** -> o `postos[]` tem 1, no maximo 2: **os 20 do contrato nao tem lastro**. `monitorar_segundo_plano` usa `ParametroSistema` por empresa (precedente de 17/09) e exige entrada em `configuracao_efeito.DECLARACAO`; NAO criar campo em `Empresa`. ETag tem de ser hash do payload (`Posto` nao tem `atualizado_em`). `turno_previsto` deve sair de `EC.marcos_do_dia` -- se sair de `janela_turno_de`, HERDA o TURNO-NATIMORTO. A frase obrigatoria: **a cerca e GATILHO, nunca VEREDITO**. | 21/09 16:35 | **30 h** | nao existe endpoint; 4 sitios de distancia; 10/205 postos sem coordenada; 49 com posto divergente | `corte Ronald: contrato GEOFENCE-API vale (monitorar em segundo plano: ligado por empresa, default desligado)` |
| 44 | aval | TURNO-NATIMORTO FLIP DO TIPO -- o aval de 21/09 17:0x nao tem conjunto para aplicar. As "172 batidas" nao existem como lista declarada (o unico 172 do RELATO e 172 CHAMADOS vivos em competencia trancada, outra medida). O juiz real (`flip_automatico --competencia`, em seco) devolve `{'dia_em_curso': 10, 'humano': 19, 'intocavel': 2}` -- **sem chave `auto`**: `--apply` mudaria 0 batidas. O que existe sao 19 batidas que o juiz mandou para DECISAO HUMANA, com motivo escrito: col923 03/09 #90374 (decisor propoe 2 flips -- ambiguo) e 09/09 #96164; col769 5; col926 4; col435 2; col92 #104630; col489 #84690; col192/col363/col859/col889 1 cada. Chamar `flip_tipo` direto nelas passa por cima do juiz que so flipa quando o flip e UNICO e deixa o dia perfeito. Nada escrito. | 21/09 17:05 | **30 h** | 19 batidas, 9 colabs | `aval Ronald: flip vale sobre as 19 (ou: aponte a lista de ids)` |
| 45 | corte | A ATA VENCE A CELULA, e a porta da celula nao fecha a cadeia. `escala/services/leitor_celula.py:344`: `_tipo = ata.get('tipo_dia') or (...cel.trabalha...)` -- `cel.trabalha` e so o FALLBACK. Em 21/09 regenerei 107 celulas para folga e o supra_juiz seguiu lendo `tipo_dia='trabalho'` / `previsto=480` da ata velha: os 41 chamados sem lastro NAO morreram. FECHADO A MAO no mesmo dia (cartorio escopado com `julgar_colab` nas 82 celulas -> 122 julgadas, 40 reconciliados, 41 -> 0 chamados, ZERO chamado novo), e o DRY provou que o cron das 06:28 faria o mesmo sozinho (identico com e sem `--forcar`: a impressao muda com a celula). Entao a pergunta que sobra NAO e' 'como resolver este caso' -- e' de desenho: (1) `regenerar_celulas_vinculo` passa a pedir o re-carimbo das celulas que mudou, e a cadeia celula->ata->chamado fecha na PORTA (1 escritor, efeito completo no mesmo ato); ou (2) fica como esta e a lampada espera o cron -- e isso vira linha escrita no CLAUDE.md, para a proxima pessoa nao repetir a minha surpresa de hoje. | 21/09 17:45 | **29 h** | 82 atas; 41 chamados (fechado a mao 21/09 18:2x) | `corte Ronald: a porta da celula re-carimba (ou: espera o cron das 06:28)` |
| 46 | aval | O QUE SOBROU DA GERADOR-FOTO (passivo FECHADO em 21/09: 89 -> 10 celulas, 41 -> 0 chamados). As 10 que restam sao exatamente as que a guarda `HX-REGEN-NAO-TOCA-EXPORTADO` barrou -- competencia 08/2026 ja emitida no TXT do Dominio: col107 03/08, 10/08, 17/08; col152 25/07, 26/07; col220 29/07, 12/08; col476 01/08, 08/08, 15/08. A guarda esta CERTA (dia que virou folha nao se toca) e nao sumiu calado: voltou em `barrados` com texto. Para alcanca-las existe porta formal -- `apesar_da_lavra='<motivo>'`, que grava o motivo na trilha de cada celula. Isso e passado JA PAGO: so com a sua palavra, e provavelmente nunca. | 21/09 17:30 | **29 h** | 10 celulas, 4 colabs | `aval Ronald: deixar as 10 como estao (ou: apesar_da_lavra com motivo X)` |
| 47 | corte | REALIZADO DE 3.121 MINUTOS NUM DIA (achado de passagem ao fechar a GERADOR-FOTO, 21/09 18:2x). `col371 29/08`: celula com `realizado=3121, previsto=420` -- **52 horas num dia** -- e protestos `FURO_PARCIAL` + `REALIZADO_INFLADO`. E' par de batidas que atravessa dias sem fechar, a mesma classe que o placar ja conta (`REALIZADO_INFLADO` = 51 na competencia encerrada da emp2, 6 na emp3, 3 na emp4). NAO mexi: e' zona de dinheiro e a competencia esta em fechamento (janela de 15/09). O numero entra na folha por `minutos_realizados_do_dia`, entao 52 h num dia nao e so feiura de relatorio. Depois do export: medir a frota inteira, decidir se o teto e por JORNADA PREVISTA x fator ou se o par sem fechamento vira furo, e curar com DIFF na sombra. | 21/09 18:25 | **28 h** | 1 dia medido; 51 na frota (emp2, competencia encerrada) | `corte Ronald: realizado inflado entra na fila depois do export (ou: antes, com DIFF)` |
| 48 | corte | A LINHA DO REGISTRO ESTA VELHA -- O SITIO JA LE O JUIZ. O motivo gravado diz 'feriado = folga na tela, ate de quem trabalha em feriado', e isso NAO acontece mais. Cadeia lida ao vivo: colaboradores/services/calendario.py:384-390 chama `ponto.precedencia.fatos_do_periodo` uma vez para a janela; :474 faz `_faixa = _fd.get('faixa')`; :107 (`if faixa == 'feriado':`) apenas MAPEIA o veredito do juiz para o status da tela, nao re-deriva a supressao. Quem decide se o feriado vence e' ponto/precedencia.py:243 (`feriado_so_no_pagamento = bool(feriado and previsto is True and getattr(ec, 'trabalha_em_feriado', False))`), que tira o feriado de `presentes` -- entao o 12x36 com trabalha_em_feriado=True em dia previsto sai com faixa 'trabalho' e cai no caminho normal (falta/ok/irregular), nunca em 'folga'. `fatos_do_dia` e' a mesma implementacao desde 16/09 (precedencia.py:161-177). Linha do tempo: a clausula entrou no juiz em 64fff072 (14/09 05:02, PRECEDENCIA-X-GRADE 4b, corte Ronald 13/09) e a tela inteira passou a ler a lei A2 em 08862aac (16/09, CALENDARIO-UM-JUIZ); a linha do registro nasceu em d5c0155f (14/09 08:27, S6-JUIZ-FERIADO), descrevendo a tela de antes. O juiz ja tem selo que MORDE: ponto/tests/test_a2_precedencia.py:278 `test_MORDE_12x36_que_trabalha_em_feriado_segue_previsto`. Conferido tambem contra a working tree: o diff nao-commitado de calendario.py toca o rotulo de folga e o veredito (:124-155), nao o ramo do feriado. POR ISSO NAO FABRIQUEI FATIA: a cura nao existe -- um selo novo ficaria VERDE na arvore original E na curada, e selo que nao morde nao prende nada (regra do selo anti-vacuidade). O QUE DE FATO SOBRA PARA O SEU OLHO, e nao e' desta linha: a clausula do juiz decide `previsto` por `eh_dia_trabalho_calculado` (aritmetica do VINCULO), nao pela CELULA. Se as duas divergirem num feriado, o juiz volta a dizer 'feriado' e a tela volta a pintar folga -- mas isso e' item da familia celula/precedencia, no juiz, nao no calendario. Nao mexi. | 22/09 10:23 | 12 h | 1 linha do registro (core/juizes.py:291, PENDENTES['feriado/prazo'], _F2 'o feriado suprime o trabalho do dia?', sitio colaboradores/services/calendario.py); 0 sitios a curar. FONTE: o codigo VIVO em HEAD + a working tree. UNIDADE: linha de pendente. UNIVERSO: so esta linha. EXCLUSOES: as outras 3 linhas de _F2 (fila_por_causa, detectar_ausencias, reavaliar_ausencias_feriado) nao foram olhadas nesta corrida. | `corte Ronald: apagar a linha do registro (core/juizes.py:291) e decrementar o contador de test_contract_juiz_tela (ou: manter, com motivo novo)` |
| 49 | corte | NAO FABRIQUEI FATIA porque _F7 ('qual o prazo do DP para responder a justificativa?') NAO TEM JUIZ DECLARADO: esta em SEM_JUIZ['feriado/prazo'] (core/juizes.py:207 -- 'nenhuma: a tela diz 1/2/3/7 dias por tipo e o cron alerta so depois de 7'), e o cabecalho do S6 repete (core/juizes.py:66, '4 SEM juiz ... prazo do DP'). Declarar autoridade e corte seu, nao meu. A segunda linha do mesmo sitio, sob _TT3 (core/juizes.py:657), diz em letra 'deveria ler nenhum'. O que li ao vivo, e o motivo de nao haver juiz e real: HA DUAS LEIS VIVAS E DISCORDANTES. (1) ponto/views.py:1115-1118, dentro de `fila_justificativas`, tabela por TIPO em DIAS -- esqueci_entrada/esqueci_saida 1, batida_errada/batida_duplicada 2, hora_extra/banco_horas/folga_trabalhada/troca_turno 3, outro 7, default 3 -- que pinta `sla_estourado` (dias > limite) e `sla_urgente` (dias >= limite) para o DP. (2) o cron `alertar_justificativas_sla` (config/crons.py:248, 07:45), que em alertar_justificativas_sla.py:16 usa 7 dias FIXOS para todo tipo: o push ao DP so sai no 7o dia, entao um 'esqueci_entrada' (limite 1 na tela, vermelho quando `dias > limite`, isto e' no dia 2) fica CINCO DIAS vermelho na tela -- dia 2 ao dia 6 -- sem que ninguem seja avisado, e um 'outro' (limite 7) recebe o push no dia 7, um dia ANTES de estourar na tela (dia 8). (3) o cartao de backlog da mesma tela (views.py:1137-1139) conta pendentes ha >= 7 dias -- terceiro numero, que nao e nem a tabela da tela nem o alerta, so coincide com o do cron. FALSO CANDIDATO, para ninguem 'curar' isto apontando para ele: `chamados/catalogo/motor.py::sla_limite_horas` e o juiz de _C9, outra ENTIDADE (Chamado), outra CHAVE (urgencia, nao tipo) e outra UNIDADE (HORAS: 1/24/48) -- apontar _F7 para ele nao e um juiz por pergunta, e trocar de pergunta. A cura barata que DESCARTEI (mover a tabela da tela para uma constante e o cron le ela) nao e neutra: ela escolhe, sem o seu corte, que o prazo por tipo vence -- e nesse instante o push ao DP passa de 7 dias para 1 dia nos dois tipos mais comuns, ou seja, MUDA O VOLUME DE ALERTA EM PROD, que e efeito de negocio e nao refatoracao. O caminho inverso (tudo 7 dias fixos) apaga a tabela por tipo que o DP ve hoje na tela. NAO MEXI EM NADA. | 22/09 13:02 | 10 h | 3 sitios do _F7 + 1 linha duplicada sob _TT3; 0 curados. FONTE: o codigo VIVO em HEAD (ponto/views.py, ponto/management/commands/alertar_justificativas_sla.py, core/juizes.py). UNIDADE: sitio que responde a pergunta _F7. UNIVERSO: as linhas do registro com pergunta _F7 mais a linha _TT3 de mesma impressao. EXCLUSOES: nao contei os leitores de `sla_estourado`/`sla_urgente` no template nem a tela do app; nao olhei _F8 (prazo de arquivo, esse TEM juiz). | `corte Ronald: _F7 e a tabela por TIPO em dias (tela, vira constante e o cron le ela) OU 7 dias fixos para todos (cron, e a tela para de pintar por tipo)? Declarado o juiz em core/juizes.py, os 3 sitios + a linha _TT3 saem numa fatia so` |
| 50 | corte | NAO FABRIQUEI FATIA porque _F9 ('o que e dia util?') NAO TEM JUIZ DECLARADO: esta em SEM_JUIZ['feriado/prazo'] (core/juizes.py:208 -- 'nenhuma: seg-sex sem feriado nem escala (VT, VA, janela comercial do escalonamento)'), e o cabecalho do S6 repete (core/juizes.py:66, '4 SEM juiz ... dia util'). Declarar autoridade e corte seu, nao meu. Alem disso o sitio do item JA ESTA CURADO ate onde cura sem lei: `colaboradores/models.py:861 dias_uteis_do_mes(ano, mes)` NAO e uma re-derivacao, e o PONTO DE FUSAO das tres copias -- a propria docstring dela diz em letra ('esta funcao so junta as tres copias da mesma conta', S6 fatia 1), o cabecalho do core/juizes.py registra 'conta de dia util do VT (3)' entre os curados da S6 fatia 1, e ela ja tem selo que MORDE em ponto/tests/test_contract_juiz_feriado.py:75-82 (`test_dia_util_do_vt_e_a_mesma_conta`, que crava 2026/9 == 22 e confere fev de ano bissexto e nao-bissexto). O motivo do proprio item diz isso ('tres copias fundidas aqui, nao e lei'). Nao ha para onde redirecionar a pergunta: a unica coisa que a fatia poderia fazer e APONTAR _F9 para esta funcao, e isso e declarar a lei 'dia util = seg-sex, feriado nao conta' -- decisao de negocio que nao esta escrita, e que tem efeito em dinheiro de VT (custo_mensal = valor_passagem x viagens_dia x dias_uteis) e na janela comercial do escalonamento de chamados. NAO MEXI EM NADA (nem em core/juizes.py, nem no contador 22 de test_contract_juiz_feriado.py: a linha so sai quando uma cura entra). | 22/09 13:07 | 9 h | 2 sitios do _F9 no registro (core/juizes.py:310 colaboradores/models.py::dias_uteis_do_mes, zona tela; core/juizes.py:311 escalonar_chamados_supervisao.py:25 `dt.weekday() < 5 and 8 <= dt.hour < 18`, zona cobranca); 0 curados nesta corrida. FONTE: o codigo VIVO em HEAD + a working tree. UNIDADE: sitio que responde a pergunta _F9. UNIVERSO: as 2 linhas de _F9 em PENDENTES['feriado/prazo']. EXCLUSOES: nao toquei o sitio do escalonamento (o item de hoje era so o de models.py); o censo de consumidores da conta foi por grep de `dias_uteis` no app e achou 3 chamadores (LinhaTransporte.custo_mensal em colaboradores/models.py:693, inteligencia/views.py:147 e :451) -- nao varri folha/ nem holerite por outro nome para a mesma conta. | `corte Ronald: _F9 e (a) seg-sex e pronto -- a funcao que ja existe vira JUIZES['feriado/prazo'][_F9] e o escalonamento passa a chamar ela: ZERO efeito em prod, so nome; (b) dia util EXCLUI feriado (core/feriados.py::eh_feriado) -- muda o custo de VT que a inteligencia mostra, muda a janela comercial do escalonamento em feriado, e o selo atual (2026/9 == 22) tem que mudar junto; ou (c) fica SEM_JUIZ e a linha de colaboradores/models.py SAI do PENDENTES porque ponto de fusao selado nao e re-derivacao -- so (c) impede o fabricante de sortear este mesmo item de novo amanha` |
| 51 | corte | MESMA PERGUNTA do corte ja aberto em `corte-aprovar-trancar-sem-juiz` (21/09) -- NAO estou duplicando o corte do juiz unico; esta linha so acrescenta o que o sitio sorteado hoje mostra. NAO FABRIQUEI FATIA porque _K1 ('a competencia pode ser aprovada/trancada (o que bloqueia)?') NAO TEM JUIZ DECLARADO: esta em SEM_JUIZ['fechamento'] (core/juizes.py:211 -- 'dividida: botao (turno aberto, 12x36 sem ancora, conta vazia), aprovar_colaborador (so turno), TXT (classificar_export, sem trancar) e os bloqueios do pre-fechamento (pauta, nao trava)'), e JUIZES['fechamento'] (core/juizes.py:185-192) so declara _K2/_K3/_K4/_K5/_K7/_K8. Declarar autoridade e corte seu, nao meu. O ACHADO DE HOJE, dentro de UMA unica funcao (ponto/views.py::fechamento_mensal), e que a tela responde a pergunta DUAS vezes com reguas diferentes: (a) ponto/views.py:631-633 `pode_aprovar = total_aprovados == 0 and all(turnos_abertos == 0)` -- o que o template consome no gate do botao (templates/ponto/fechamento.html:208, `{% if pode_aprovar and empresa_id and not periodo_fechado %}`); (b) ponto/views.py:668-673 `n_limpos_pend`, a PILULA, cujo comentario diz em letra 'MESMA regua do aprovar_fechamento (C2) ... Pilula e botao nao podem divergir do backend (BO 31/07)' e que filtra aberto + 0 turno aberto + fora da lista 12x36-sem-ancora + conta NAO vazia. As duas divergem hoje: basta UM fechamento 'aprovado' na competencia para o botao sumir (total_aprovados == 0) enquanto a pilula segue contando N prontos -- e a acao (ponto/views.py:716+ aprovar_fechamento) continuaria aprovando os 'aberto' restantes se fosse chamada. E o botao nao olha ancora nem conta vazia, que a pilula e a acao olham; o template ainda acrescenta um TERCEIRO pedaco de resposta (`empresa_id and not periodo_fechado`). NAO MEXI EM NADA: nem em ponto/views.py, nem em core/juizes.py, nem no contador de core/tests/test_contract_juiz_tela.py -- a linha so sai quando uma cura entra. | 22/09 13:11 | 9 h | 1 sitio do _K1 lido nesta corrida (core/juizes.py:320, ponto/views.py:631 `pode_aprovar = total_aprovados == 0 and all(`, zona tela); 0 curados. FONTE: o codigo VIVO em HEAD + working tree (ponto/views.py:631-633, 663-673; templates/ponto/fechamento.html:208; ponto/views.py:716-775 aprovar_fechamento; core/juizes.py:185-192 e 210-213). UNIDADE: sitio que responde a pergunta _K1. UNIVERSO: as 6 linhas de _K1 em PENDENTES (5 em ponto/views.py, 1 em folha/views.py). EXCLUSOES: nao li nem toquei os 5 sitios de DINHEIRO do _K1 (lote vazio, 12x36 sem ancora, aprovar_colaborador, tranca por empresa, TXT em folha/views.py); nao contei fechamentos em prod -- o '592 abertos' e do registro de 21/09, nao medida minha. | `corte Ronald (estreito, cabe antes do juiz unico): `total_aprovados == 0` no gate do botao e LEI ('lote so na PRIMEIRA aprovacao da competencia; depois so linha a linha') ou e BUG? Se BUG: `pode_aprovar = n_limpos_pend > 0` e fatia de TELA (sem dinheiro), o botao passa a bater com a pilula e com a acao, e a impressao sai do registro -- mas ATENCAO, isso NAO cura o _K1 (a pilula tambem re-deriva a regua), so apaga esta copia. Se LEI: o comentario de ponto/views.py:663-665 ('pilula e botao nao podem divergir') esta ERRADO e precisa dizer a lei, e o sitio fica parado atras do corte grande.` |
| 52 | corte | NAO FABRIQUEI FATIA, e aqui NAO e por falta de juiz: _K2 TEM autoridade declarada (JUIZES['fechamento'][_K2] = folha/export.py::classificar_export). O sitio re-deriva a resposta em 12 linhas de Python copiando a ORDEM do juiz pela metade -- rescisao -> sem_codigo -> nao_medido -> furo_espelho -- enquanto o juiz decide rescisao -> sem_codigo_dominio -> ferias_com_batida -> furo_espelho -> nao_homologado. EFEITO VISIVEL HOJE: colaborador que bateu ponto DENTRO de ferias agendadas, ou com codigo de rubrica fora do catalogo Dominio, recebe TIQUE VERDE na tela de fechamento (titulo 'ENTRA na folha desta competencia (mesmo juiz do TXT)') e o TXT o RETEM. O comentario do proprio arquivo (:59-60) diz que 'a tela declara a conferir' para nao_homologado -- isso NAO esta no markup: templates/ponto/_fech_linha.html:11 so tem o ponto cinza 'ainda nao medido' para `motivos_espelho is None`; no resto o tique e verde e liso. NAO MEXI EM NADA: nem em ponto/services/fechamento_lista.py, nem em core/juizes.py, nem no contador de core/tests/test_contract_juiz_tela.py. OS DOIS BLOQUEIOS, independentes: (1) FRONT -- o badge de templates/ponto/_fech_linha.html:11 tem TRES ramos (`furo_espelho` / `sem_codigo` / else) e o else escreve 'rescisao': qualquer motivo novo que o juiz devolva sai rotulado RESCISAO na tela do admin. Fatia de tela que toca template exige smoke de clique seu (BUG 73), que esta corrida nao tem. DESCARTEI a cura barata de disfarcar `ferias_com_batida` como `furo_espelho` e esconder a verdade no `detalhe` do title -- e trocar uma mentira por outra, meia-correcao. (2) PERF DECLARADA -- o modulo existe com a regra escrita 'o pedaco caro (grade) ja vem MATERIALIZADO em fech.motivos_espelho; zero grade por colab aqui' (:56-58) e os avisos 31/07 (:42-45, :48-51, medido la: 6424 queries / 4.1s). Chamar classificar_export acrescenta, POR COLABORADOR: `fech.colaborador.empresa` (+1, o juiz so faz select_related('colaborador')), CelulaDia + EscalaColaborador dentro de motivos_retencao_celula (+2) e, com catalogo Dominio cadastrado, _ciclo_do_colab + regua_para dentro de feriado_12x36_em_dobra (+2) -- ~4 a 5 queries/colab vezes as 436-570 linhas que a pagina monta (hoje: 0). Ainda: sao DOIS UNIVERSOS -- o juiz varre FechamentoMensal da EMPRESA INTEIRA, a tela monta qs_colab por SITUACAO (ponto/views.py:543-547: ativo, ou desligado com data_demissao >= inicio da janela) -- entao ele trabalharia tambem para quem nao esta na tela; e o estado 'nao_medido' (o ponto cinza) MORRE, porque classificar_export sempre mede. | 22/09 13:16 | 9 h | 1 sitio, DUAS linhas no registro para o MESMO bloco: core/juizes.py:321 (_K2 'quem entra no TXT da folha?', impressao `_folha = {'entra': False, 'motivo': 'rescisao'}`) e core/juizes.py:489 (_TT6 'o colaborador entra na folha (e por que nao)?', impressao `if fech and empresa_id:`) -- as duas apontam ponto/services/fechamento_lista.py:61-73, zona TELA. 0 curados. FONTE: codigo VIVO (ponto/services/fechamento_lista.py:52-90; folha/export.py:371-440 classificar_export, :573-635 motivos_retencao_celula, :637-642 motivos_retencao_espelho, :205-220 _ciclo_do_colab/feriado_12x36_em_dobra, :282-322 dias_ferias_com_batida; templates/ponto/_fech_linha.html:11; ponto/views.py:554 e 617). UNIDADE: queries por colaborador ADICIONAIS as de hoje (contadas por leitura de codigo, NAO medidas em prod). UNIVERSO: a pagina de fechamento com empresa escolhida (a tela so calcula `folha` quando `empresa_id`; em 'Todas' o tique nem aparece). EXCLUSOES: nao contei colaborador nenhum em prod -- quantos hoje recebem tique verde e seriam retidos por ferias_com_batida/nao_homologado esta NAO MEDIDO; nao li o custo do `prontidao`, que ja roda na mesma passada e nao muda com a cura. | `corte Ronald, duas perguntas na mesma folha: (A) a tela de fechamento pode PAGAR o juiz (classificar_export uma vez por pagina, ~4-5 queries/colab x 436-570, e o ponto cinza 'nao medido' deixa de existir porque o juiz sempre mede)? Se SIM, a fatia e de TELA, cabe na janela de fechamento, e precisa ir junto o template com DOIS badges novos (ferias / sem cod catalogo) + seu smoke -- sessao interativa, nao esta corrida. Se NAO: (B) o caminho e materializar `ferias_com_batida` e `nao_homologado` ao lado de `motivos_espelho` no recalculo (ponto/services/fechamento.py = DINHEIRO, campo novo = migration), e ai a fatia esta BARRADA pela janela ate o export de 09/2026 confirmado. Enquanto nenhuma das duas for escolhida, o fabricante vai sortear este mesmo bloco de novo (sao DUAS linhas, _K2 em core/juizes.py:321 e _TT6 em :489, para uma cura so).` |
| 53 | corte | A LINHA DO REGISTRO ESTA VELHA -- O LED JA LE O JUIZ. colaboradores/painel_cell.py:64-66 decide 'amarelo' por `turno_aberto_de(c, agora=timezone.now()) is not None` desde 21/09 (comentario LED PELO JUIZ, :55-63), e o selo colaboradores/tests/test_led_pelo_juiz.py ja MORDE (test_RED_ultima_batida_E_nao_e_turno_aberto x test_MORDE_com_turno_de_verdade_o_LED_continua_amarelo, caso col923). A impressao gravada (:32 `Batida.objects.filter(colaborador_id=c.pk, timestamp__gte=corte)`) continua no arquivo, mas hoje NAO responde _TT1: alimenta so 'ultima_batida' (exibicao de marcacao -- le o cru por lei, Portaria 671), o pino de geo (_geo_status_pino) e o degrau verde/cinza do LED ('bateu nas ultimas 14 h' x 'nao bateu'), que nao e pergunta de turno aberto. POR ISSO NAO FABRIQUEI FATIA: a cura nao existe, e um selo novo ficaria VERDE na arvore original e na curada. Apagar a linha sem cura tambem nao fiz -- a lista so encolhe por fatia, e o contador de test_contract_juiz_tela.py e seu. | 22/09 13:25 | 9 h | 1 linha do registro (core/juizes.py:374, PENDENTES['tela'], _TT1 'quem esta em turno (turno aberto vivo) agora?', sitio colaboradores/painel_cell.py); 0 sitios a curar. FONTE: o codigo VIVO na working tree (painel_cell.py nao aparece no git status). UNIDADE: linha de pendente. UNIVERSO: so esta linha. EXCLUSOES: as outras linhas de _TT1 (painel_op, core/views.py, templates) nao foram olhadas; o modo LOTE (views.painel_situacional -> maps['batida_recente']) nao foi lido. | `corte Ronald: apagar a linha _TT1 de painel_cell.py em core/juizes.py:374 e decrementar o contador de test_contract_juiz_tela (o LED ja le turno_aberto_de, selado em test_led_pelo_juiz) -- ou, se o degrau verde/cinza ('bateu em 14 h') tambem deve ter juiz, declarar qual; sem isso o fabricante sorteia este item de novo` |
| 54 | corte | NAO FABRIQUEI: O ITEM JA TEM FATIA. .esteira/k8_dashcomp (DASH-SUPERVISOR-COMPETENCIA) cura exatamente este sitio pelo juiz (janelas.fechamentos_da_competencia_corrente, Q por empresa), passou RED failures=2 / GREEN 913 as 14:03, foi entregue ao integrador e caiu com fatia.done ESTADO=CAIU_ARVORE 'baseline divergiu: a arvore andou depois do teste da fatia'. Continua em logs/fila_esteira.txt:49. Cheguei a montar .esteira/fm_supervisor com a mesma cura e APAGUEI antes de o fabricante.sh a ver -- duas fatias no mesmo sitio colidiriam no baseline uma da outra. A linha segue em core/juizes.py porque a k8_dashcomp nao commitou. | 22/09 14:30 | 8 h | 1 linha do registro (core/juizes.py:328, PENDENTES['fechamento'], _K8 'que FechamentoMensal e o da competencia de hoje?', sitio core/views.py::dashboard_supervisor). FONTE: .esteira/k8_dashcomp/{esteira.out,fatia.done,msg_commit.molde} e logs/fila_esteira.txt:49, lidos ao vivo. UNIDADE: fatia. UNIVERSO: so este item. EXCLUSOES: as irmas k8_detalhe360/k8_scores/k8_alertas/k8_golive nao foram abertas. | `corte Ronald: relancar .esteira/k8_dashcomp (CAIU_ARVORE, so baseline) -- e o fabricante.sh deveria pular item cujo sitio ja tem esteira viva/caida em logs/fila_esteira.txt, senao sorteia de novo o que ja esta na fila` |
| 55 | corte | NAO FABRIQUEI: O ITEM JA TEM FATIA. .esteira/score_fmcomp (SCORE-FM-COMPETENCIA) cura exatamente este sitio (construir.py tira a mesma linha de juizes.py), passou RED failures=1 / suite 978 as 14:26 e esta com fatia.done ESTADO=PRONTA, 'entregue ao lote (o integrador commita)'. O sitio segue vivo em calcular_scores.py:58 so porque o integrador ainda nao commitou. Nada foi escrito em .esteira nem em app/ alem desta linha. | 22/09 14:40 | 8 h | 1 linha do registro (core/juizes.py:328, PENDENTES['fechamento'], _K8, sitio inteligencia/management/commands/calcular_scores.py). FONTE: .esteira/score_fmcomp/{fatia.done,esteira.out,msg_commit.txt,construir.py} e logs/fila_esteira.txt:54, lidos ao vivo. UNIDADE: fatia. UNIVERSO: so este item. EXCLUSOES: nenhuma outra esteira aberta. | `mesmo corte da k8-dashcomp: o fabricante.sh deveria pular item cujo arquivo ja aparece numa esteira de logs/fila_esteira.txt (aqui score_fmcomp, PRONTA) -- sorteou pela segunda vez hoje um item ja em fila` |
| 56 | corte | NAO FABRIQUEI: O ITEM JA TEM FATIA. .esteira/k8_golive (GOLIVE-COMPETENCIA) cura exatamente este sitio, passou RED failures=1 / suite 3033 as 14:22 e esta com fatia.done ESTADO=PRONTA, 'entregue ao lote (o integrador commita)'. O sitio segue vivo em checklist_golive.py:112 (mes=hoje.month) so porque o integrador ainda nao commitou. Nada escrito em .esteira nem em app/ alem desta linha. | 22/09 15:05 | 8 h | 1 linha do registro (core/juizes.py:330, PENDENTES['fechamento'], _K8, sitio ponto/management/commands/checklist_golive.py:112). FONTE: .esteira/k8_golive/{fatia.done,esteira.out,pronta.json,msg_commit.molde} e logs/fila_esteira.txt:53, lidos ao vivo. UNIDADE: fatia. UNIVERSO: so este item. EXCLUSOES: nenhuma outra esteira aberta. | `terceira vez hoje (k8_dashcomp, score_fmcomp, k8_golive): o fabricante.sh deveria pular item cujo arquivo ja aparece numa esteira de logs/fila_esteira.txt` |
| 57 | corte | NAO FABRIQUEI: o proprio motivo do registro diz 'deveria ler sem juiz declarado' -- e a regra do fabricante e parar ai (juiz novo e corte seu). Alem disso a linha esta MAL CLASSIFICADA: o sitio (`op_justif = _Just.objects.filter(status='pendente').count()`) conta Justificativa pendente para o card do painel situacional; nao responde _TT6 'o colaborador entra na folha', cujo juiz (folha/export.py::classificar_export) classifica colab a colab para o TXT e nao conta justificativa. Trocar por classificar_export seria cura falsa (o selo nao morderia pelo motivo certo) e ainda entraria em zona de dinheiro. Nenhum juiz declarado responde 'quantas justificativas estao pendentes'. Saidas: (1) declarar juiz de 'justificativa pendente' (ex.: um queryset/contador em ponto/ que as telas e o copiloto consultem) e re-registrar a linha sob essa pergunta; (2) re-classificar a linha como 'sem juiz, contagem crua aceita' e tira-la do registro. Nada escrito em .esteira nem em app/ alem desta linha. | 22/09 15:30 | 7 h | 1 linha do registro (core/juizes.py:375, PENDENTES['tela'], _TT6, sitio colaboradores/services/situacional.py:136). FONTE: core/juizes.py:350,362,375-376 e situacional.py:135-136, lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma. | `corte Ronald: juiz de 'justificativa pendente' (ou: tirar a linha situacional.py#1 do registro)` |
| 58 | corte | NAO FABRIQUEI: o motivo do registro diz 'deveria ler nenhum declarado (geofence)' -- juiz novo e corte seu. A linha tambem esta MAL CLASSIFICADA: `_geo_status_pino` (views.py:2295) calcula haversine da coord da ultima batida contra o posto e devolve verde/vermelho/cinza pelo raio; nao responde _TT2 'pares de batida', e turnos_do_colab/parear_turnos nao cabem. Candidato natural a juiz existe mas NAO esta declarado: ponto/services/geofence.py (calcular_distancia + a regra `<= (raio_metros or 200)` de alerta_caducou). Ha pelo menos 6 haversines/raios soltos no repo (lista em registros). Saidas: (1) declarar em core/juizes.py uma familia/pergunta 'a coordenada cai dentro do raio do posto?' com autoridade em ponto/services/geofence.py (ex.: extrair `dentro_do_raio(lat, lon, posto)`), re-registrar esta linha sob ela e so entao fabricar; (2) tirar a linha de _TT2 e marca-la sem juiz (pino de tela, cor apenas). Nada escrito em .esteira nem em app/ alem desta linha. | 22/09 16:00 | 7 h | 1 linha do registro (core/juizes.py:377-378, PENDENTES['tela'], _TT2, sitio colaboradores/services/situacional.py:124). FONTE: core/juizes.py:377-378, situacional.py:36,124, colaboradores/views.py:2295-2310 e ponto/services/geofence.py:35-55, lidos ao vivo; grep '6371\|raio_metros or' em app/ fora de tests/migrations. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: os outros sitios de distancia achados no grep (api/views.py:2127, api/views_core.py:964, ponto/views.py:2128, reconciliar_vinculo.py:57, detectar_vinculo_divergente.py:19) foram so listados, nao julgados. | `corte Ronald: juiz de geofence 'dentro do raio do posto' em ponto/services/geofence.py (ou: tirar situacional.py#3 de _TT2)` |
| 59 | corte | NAO FABRIQUEI: a cura nao e mecanica nas duas metades do motivo. (A) TURNO EM CURSO -- o sitio ja NAO re-deriva: desde F1 TURNO tela sitio 6 (14/09) 'em turno' sai de `turnos_em_turno_stored(agora)` (painel_op.py:53-54,113), que le o TurnoMaterializado(aberto) GRAVADO pelo nucleo com liveness ancorada na entrada; a linha :120 so calcula a duracao a partir da entrada desse turno gravado. Trocar por `turno_aberto_de` (que RECALCULA por colab, _turno_aberto_calc) seria ~750 recalculos por render numa tela que S4 (02-03/09) otimizou justamente contra N+1, e as duas leituras podem divergir (stored+liveness x calc). Qual e a autoridade de _TT1 em LOTE -- turnos_em_turno_stored como face em lote do juiz, ou turno_aberto_de sempre -- e declaracao sua. (B) AUSENTE/FOLGA -- hoje e `eh_dia_trabalho` (juiz declarado de 'dia de trabalho', S2) + 'sem entrada' => 'ausente' (painel_op.py:137-141). Ler `fatos_do_dia` muda o que a tela MOSTRA: colab de ferias/atestado/afastado/feriado em dia de escala hoje sai 'ausente'; pela precedencia ele deixaria de ser ausente, e para onde vai (folga? status novo 'ferias'/'afastado'?) nao esta escrito -- status novo tambem mexe em contadores, `statuses` e no template (front, fora desta corrida). Saidas: (1) declarar turnos_em_turno_stored como face em lote de _TT1 (a metade A sai do registro sem codigo) e cortar o rotulo da metade B (ex.: fatos_do_periodo com status 'ferias'/'afastado' novos, com smoke); (2) reclassificar a linha para _TT3/dia (a metade A ja esta curada) e manter B pendente; (3) manter. Nao toquei core/juizes.py nem o contador de core/tests/test_contract_juiz_tela.py. NAO MEDIDO: quantos colabs hoje aparecem 'ausente' estando cobertos por ausencia/ferias/feriado (consulta na sombra nao autorizada nesta corrida) -- FONTE fatos_do_periodo x eh_dia_trabalho na sombra / UNIDADE colab-dia / UNIVERSO ativos dos 3 CNPJs, hoje / EXCLUSOES quem tem entrada. | 22/09 17:00 | 6 h | 1 linha do registro (core/juizes.py:383-384, PENDENTES['tela'], _TT1, sitio core/services/painel_op.py:120 'delta = agora - _tm.entrada.timestamp'). FONTE: painel_op.py:53-54,98-141, ponto/selecao_periodo.py:174-210 (turnos_em_turno_stored), ponto/turnos.py:757-766 (turno_aberto_de), ponto/precedencia.py:161-178 (fatos_do_dia/fatos_do_periodo), lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: as linhas irmas painel_op.py 'contadores = {' (_TT6) e 'presente = sum(...)' (_TT3) nao foram julgadas. | `corte Ronald: turnos_em_turno_stored vale como juiz de _TT1 em lote (ou: turno_aberto_de sempre) + rotulo de ferias/afastado no painel operacional` |
| 60 | corte | NAO FABRIQUEI: a linha esta MAL CLASSIFICADA e a cura nao tem alvo proprio. Os 6 contadores (painel_op.py:163-170) nao respondem 'entra na folha' e nao re-derivam nada: sao sum() do campo `status` que o laco de painel_op.py:98-141 ja calculou por colaborador (normal/geofence/turno_aberto/ausente/encerrado/folga). Trocar por folha/export.py::classificar_export (a autoridade de _TT6) nao faz sentido -- o painel operacional e o AGORA do posto, nao elegibilidade de folha (e folha/ e zona de dinheiro, fora desta corrida). O proprio motivo diz 'deveria ler idem', isto e, o mesmo juiz da linha anterior (status por colab, _TT1 + fatos_do_dia), que ja esta PARADO no corte 'corte-painel-op-status-tt1'. Quando o status for curado, os contadores ficam certos sem tocar esta linha. Saidas: (1) tirar painel_op.py 'contadores = {' do registro (e decrementar test_contract_juiz_tela) por ser agregacao do status, sem pergunta propria; (2) reclassificar para _TT1 e deixar cair junto com a cura do status; (3) manter. Nao toquei core/juizes.py nem o contador do selo. | 22/09 17:30 | 5 h | 1 linha do registro (core/juizes.py:385-386, PENDENTES['tela'], _TT6 'o colaborador entra na folha (e por que nao)?', sitio core/services/painel_op.py:163 'contadores = {'). FONTE: core/juizes.py:348,360,385-386 e painel_op.py:98-170, lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: a linha irma 'presente = sum(...)' (_TT3) nao foi julgada. | `corte Ronald: tirar painel_op.py 'contadores = {' do registro (agrega o status, cai junto com corte-painel-op-status-tt1) ou reclassificar para _TT1` |
| 61 | corte | NAO FABRIQUEI: a chave `em_turno` do contexto de core/views.py::dashboard e MORTA -- nenhum template a le. dashboard2.html (o unico template do view; `_dash_template` nao e setado em lugar nenhum fora de testes) nao menciona em_turno, nao tem include, e base.html/partials tambem nao leem `{{ em_turno }}` (o `em_turno` de _painel_card.html e `p.em_turno`, e painel_tv.html le `total_em_turno` -- outros views). Curar pelo juiz seria pagar N x turno_aberto_de (~2 queries por candidato, centenas de colabs com batida em 3 dias) na PAGINA DE ENTRADA do admin para computar um numero que ninguem ve; o selo so morderia o contexto, nao a tela. Nao e decisao minha entre (1) APAGAR a chave morta (e a linha `em_turno = len(_em_turno_ids)`) e tirar a linha do registro por falta de consumidor -- decremento de test_contract_juiz_tela.py 150->149 e grupo 1 22->21; (2) curar pelo juiz mesmo assim (receita pronta: candidatos = batida nao retratada em 3 dias, turno_aberto_de(c, agora), como colaboradores/services/situacional.py:94-97) e passar a MOSTRAR o numero em dashboard2.html (front, precisa smoke); (3) manter. Nao toquei core/views.py, core/juizes.py nem o contador do selo. | 22/09 16:04 | 7 h | 1 linha do registro (core/juizes.py:390-391, PENDENTES['tela'], _TT1 'quem esta em turno (turno aberto vivo) agora?', ancora 'chamados_emergencia = ChamadoColaborador.objects.filter(', sitio core/views.py::dashboard:206 `em_turno = len(_em_turno_ids)`). FONTE: core/views.py:150-230, templates/core/dashboard2.html (336 linhas, extends base.html, zero {% include %}) e grep de `em_turno` em templates/, lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: as linhas irmas do mesmo view (turnos_abertos/tempo_aberto, ancoras total_empresas_ativas e tempo = f...) nao foram julgadas. | `corte Ronald: apagar a chave morta em_turno do dashboard e tirar a linha do registro, ou curar pelo juiz e mostrar na tela (front com smoke)` |
| 62 | corte | NAO FABRIQUEI: a cura nao e troca mecanica por um juiz, e sim redefinir as gavetas. (1) A autoridade declarada de _TT3 e classificar_falta, que devolve 'qual batida falta NESTE dia' (tipo, motivo, verbo) -- particionar nao pergunta isso: pergunta 'bateu no dia PREVISTO VIZINHO antes/depois?' (adesao/falta_2_dias/dia_sem_declaracao/ausencia_sustentada). Nao ha como encaixar classificar_falta sem decidir o que cada gaveta passa a significar. (2) O proprio motivo cita TRES fontes (turnos_do_colab + fatos_do_dia + CelulaDia.veredito); cada uma muda o numero de um jeito: fatos_do_dia tira do 'previsto vizinho' os dias de ferias/atestado/feriado (hoje e CelulaDia.trabalha=True cru), o que move chamados entre ausencia_sustentada/falta_2_dias/dia_sem_declaracao; veredito da celula do PROPRIO dia (ex.: celula ja nao acusa) pediria gaveta nova ('a celula ja absolveu') -- gaveta nova e corte seu. (3) particionar e LOTE (1.438 chamados, uma passada); turnos_do_colab/classificar_falta sao por colab/dia -- N consultas na tela /chamados/painel/, que ja derrubou worker por timeout em 11/09. (4) A saida alimenta bloqueios_para_fechar (numero do DP para fechar competencia), na janela de fechamento. Metade mecanica possivel sem corte: trocar o Batida.objects.filter(retratada_em__isnull=True) cru por batidas_apuraveis (universo de apuracao, CLAUDE.md sec.4) -- mas isso nao tira a linha _TT3. Saidas: (a) declarar a semantica: 'previsto vizinho' = fatos_do_dia ('trabalho'), 'bateu' = turnos pareados, e o veredito da celula do dia decide uma gaveta nova 'celula ja nao acusa' antes das de falta -- entao fabricar com selo de gaveta; (b) reclassificar a linha para _TT2 (pares de batida) + eh_dia_trabalho, que e o que o codigo de fato pergunta; (c) manter. Nao toquei fila_por_causa.py, core/juizes.py nem o contador de core/tests/test_contract_juiz_tela.py. NAO MEDIDO: quantos chamados trocariam de gaveta sob (a) -- FONTE particionar() x variante na sombra / UNIDADE chamado / UNIVERSO qs_na_fila / EXCLUSOES suporte e sem dia. | 22/09 18:00 | 5 h | 1 linha do registro (core/juizes.py:414-415, PENDENTES['tela'], _TT3 'o dia acusa (falta, furo, ok) -- o que a celula julgou?', sitio chamados/services/fila_por_causa.py::particionar + _gaveta_de). FONTE: fila_por_causa.py:139-250, core/juizes.py:344,356,414-419, escala/servico_jornada.py:137-180 (classificar_falta), ponto/turnos.py:618-633 (turnos_do_colab), lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: as linhas irmas toques (_TT2) e bloqueios_para_fechar (_TT7) e a linha _F2 'if dia and dia in feriados:' nao foram julgadas. | `corte Ronald: o que as gavetas de falta passam a significar sob fatos_do_dia + veredito da celula (gaveta nova 'celula ja nao acusa'?), ou reclassificar particionar para _TT2` |
| 63 | corte | FABRICANTE 22/09, item chamados/services/fila_por_causa.py#23 ('def toques(base=None, p=None):', _TT2 'quais sao os turnos do colaborador (pares de batida)?', 'deveria ler idem'). NAO FABRICADA: lido ao vivo (fila_por_causa.py:293-326), toques() NAO pareia batida nem pergunta turno nenhum -- recebe a particao pronta (p ou particionar(base)), le dias_do_fato (chamados.juizes) e so agrupa chamados por _chave_de_toque (colab / colab+semana ISO / pk). Nao ha re-derivacao para trocar por juiz. O unico pareamento do modulo esta em particionar (fila_por_causa.py:179-189) e JA chama o juiz ponto/turnos.py::parear_turnos -- o resto dele e a linha _TT3, ja em corte (corte-fila-por-causa-particionar-tt3). 'idem' herda aquele corte. | ? | ? |  | `corte Ronald: a linha 'toques' (_TT2) de fila_por_causa sai do registro como agregador sem sitio, ou cai junto com o corte de particionar?` |
| 64 | corte | FABRICANTE 22/09, item core/views.py#12 ("postos_cobertos = sum(1 for p in postos_painel if p['cobertura'] == 'coberto')", _TT3 'o dia acusa -- o que a celula julgou?', 'deveria ler idem' = fatos_do_dia + CelulaDia.veredito). NAO FABRICADA: lido ao vivo (core/views.py:636-639), painel_operacional NAO julga nada -- so conta o campo 'cobertura' que core/services/painel_op.py::montar_painel_operacional ja decidiu (painel_op.py:221-251: classificar_cobertura sobre previsto_agora/presente/teve_atividade, mapeado por _MAP_OP). O previsto de la JA pergunta a celula desde 2788c7d0 [PAINEL-PREVISTO-CELULA] (eh_dia_trabalho + marcos_turno com celulas=), que tirou a linha _TT3 DAQUELE sitio. Nao ha re-derivacao aqui para trocar por juiz; o juizo de cobertura (se deve ou nao ler CelulaDia.veredito) mora em painel_op, nao em views. | ? | ? |  | `corte Ronald: a linha postos_cobertos (_TT3) de core/views.py sai do registro como agregador de painel_op (ja curado em 2788c7d0), ou passa a apontar para painel_op.classificar_cobertura?` |
| 65 | corte | FABRICANTE 22/09, item chamados/services/fila.py#20 ("'prazo_vencido': com_prazo.filter(prazo_sla__lt=agora).count()", _TT8 'o chamado ou a pergunta esta vivo na fila?', motivo 'deveria ler sem juiz (sla_limite_horas)'). NAO FABRICADA: o proprio motivo declara 'sem juiz' -- regra do fabricante e parar. Lido ao vivo (fila.py:148-165): o universo JA vem da porta (qs_na_fila); a re-derivacao e da pergunta de PRAZO, nao de 'vivo'. Essa pergunta tem juiz em outra familia: _C9 'o prazo (SLA) do chamado estourou?' -> chamados/catalogo/motor.py::prazo_estourou (motor.py:54, `prazo_sla <= agora`, sem prazo = nao estourou) + prazo_de_cobranca_do_chamado (motor.py:59, deriva o prazo de criado_em para o legado sem carimbo, C-SLA-PRAZO 20/09). O sitio diverge nos dois pontos: usa `<` (borda) e conta a parte 'sem_prazo_definido' como nao vencida, sem derivar -- os 27 vivos sem carimbo medidos em 20/09 ficam fora de 'prazo_vencido'. | ? | ? |  | `corte Ronald: 'prazo_vencido' do copiloto (fila.py) passa a perguntar a prazo_estourou/prazo_de_cobranca_do_chamado (linha vira _C9, legado sem carimbo entra por prazo derivado), ou fica como leitura crua do campo rotulada e a linha sai do _TT8?` |
| 66 | corte | NAO FABRIQUEI FATIA. O motivo do item diz 'deveria ler nenhum (C9 esta em SEM_JUIZ)' -- e isso esta DESATUALIZADO: _C9 tem juiz desde C-MIUDOS-1 (18/09), core/juizes.py:172 -> chamados/catalogo/motor.py::prazo_estourou. Mas o sitio JA NAO re-deriva: painel.py:376 pergunta a chamados/services/sla.py::vencido, que o corte Ronald de 15/09 declarou 'uma pergunta, um juiz' do prazo do chamado (docstring de sla.py). Sao DUAS autoridades para a mesma pergunta, com leis diferentes: sla.vencido = referencia (ultima resposta do DP, ou criado_em) + sla_limite_horas(urgencia), comparado com '<', so para status em COBRANCA; motor.prazo_estourou = campo prazo_sla (carimbado com 20 min de PRAZO_COBRANCA_MIN, derivado de criado_em no legado) comparado com '<='. Alem disso a linha esta arquivada sob _TT4 ('quantos minutos o dia realizou?'), pergunta que ela nao responde. sla_alerta (75%) e horas_sem_resposta seguem sem juiz nenhum. NAO MEXI EM NADA. RE-TRIADO 22/09 pelo fabricante (mesmo item chamados/services/painel.py#23 sorteado de novo): lido ao vivo, core/juizes.py:416-417 segue igual e _C9 segue com juiz em :172 -- nada mudou, nao fabricado. RE-SORTEADO 22/09 de novo como painel.py#22 (core/juizes.py:414-415; painel.py:376 segue perguntando a _sla.vencido): nao fabricado. Enquanto a linha ficar no registro sem corte, o fabricante vai sortea-la toda vez que a trava A cair abaixo de 3. | ? | ? | 1 linha do registro (core/juizes.py:418, sob _TT4); 0 curados. FONTE: codigo VIVO (chamados/services/painel.py:370-377, chamados/services/sla.py:17-39, chamados/catalogo/motor.py:54-72, core/juizes.py:172). UNIDADE: sitio. UNIVERSO: a linha do item. EXCLUSOES: nao olhei o template nem o cron alertar_chamados_sla. | `corte Ronald: o 'SLA estourado' do painel de gestao e o prazo de RESPOSTA DO DP (sla.py::vencido, horas por urgencia) ou o prazo da COBRANCA (motor.prazo_estourou, prazo_sla)? Se forem duas perguntas, sla.py vira juiz declarado de uma pergunta nova e a linha sai do _TT4; se for uma so, qual vence?` |
| 67 | aval | FABRICADA com escolha declarada (autorizacao previa de 22/09): .esteira/badge_navbar_consome, BADGE-NAVBAR-CONSOME-PARTICAO. Escolhi a saida (a) da corrida anterior: a linha sai de core/juizes.py como CONSUMIDOR sem sitio (contador tela -1, grupo 2 -1); o command NAO muda. Selo core/tests/test_badge_navbar_consome_particao.py: MORDE com toques/bloqueios 7/3 x 11/5 -> lavrado 7/3 x 11/5; particionar 1x com a MESMA particao aos dois; command sem geometria de turno; registro sem o consumidor (o RED na arvore anterior e este). Fatia sem prova local (a corrida nao executa python/docker): RED/GREEN acontece no rodar.sh. HISTORICO (corrida anterior, NAO FABRICADA). Lido ao vivo: lavrar_badge_navbar.py:63-65 so CONSOME a particao -- _p = particionar(); toques(p=_p); bloqueios_para_fechar(p=_p) -- e lavra os inteiros (badge_toques, badge_registrados, bloqueios_para_fechar_hoje). Nao pareia batida nem pergunta turno nenhum; nao ha re-derivacao para trocar por juiz. Reescrever o command para ler turnos_do_colab + fatos_do_dia seria uma SEGUNDA implementacao da particao (viola o C7 que a docstring do proprio command declara: contador e universo saem do mesmo lugar). O unico pareamento esta em fila_por_causa.particionar, que JA chama ponto/turnos.py::parear_turnos; a divida real dele e a linha _TT3 ja em corte (corte-fila-por-causa-particionar-tt3). Mesmo caso de corte-fila-por-causa-toques-tt2-sem-sitio. Nao toquei o command, core/juizes.py nem core/tests/test_contract_juiz_tela.py. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:424, PENDENTES['tela'], _TT2 'quais sao os turnos do colaborador (pares de batida)?', sitio core/management/commands/lavrar_badge_navbar.py '_p = particionar()'); 0 curados. FONTE: codigo VIVO (lavrar_badge_navbar.py:58-77, chamados/services/fila_por_causa.py:139-160, core/juizes.py:410-425). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nao julguei o resto do command (na_fila, placar) nem as linhas irmas de fila_por_causa. | `` |
| 68 | corte | FABRICANTE 22/09, NAO FABRICADA (duplicata). Este item JA tem fatia: .esteira/reabrir_pend_viva (REABRIR-PENDENTE-PELO-JUIZ) -- cura em reabrir_disputa_supervisao com pergunta_viva, selo test_reabrir_pendente_pelo_juiz, RED failures=2 na arvore anterior, suite 2963 verde, ESTADO=PRONTA 17:32 'entregue ao lote (o integrador commita)'. A linha continua em core/juizes.py porque o lote ainda nao foi commitado; o fabricante le o registro VIVO e sorteia de novo o que ja esta na fila do integrador. Fabricar outra fatia colidiria nas mesmas ancoras (views.py, juizes.py, contador do test_contract_juiz_tela). Nao toquei app/. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:418, PENDENTES['tela'], _TT8 'o chamado ou a pergunta esta vivo na fila?', sitio chamados/views.py 'from chamados.services.fio import _disputa_do_chamado'); 0 curados nesta corrida. FONTE: arvore VIVA + .esteira/reabrir_pend_viva/ (fatia.done, pronta.json, esteira.out) + logs/fila_esteira.txt:62. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nao auditei outras linhas do registro que possam estar no mesmo estado. | `corte Ronald: o fabricante passa a pular item cujo arquivo+ancora ja esta numa fatia PRONTA nao integrada (reabrir_pend_viva foi sorteada de novo 22/09)?` |
| 69 | corte | FABRICANTE 22/09, NAO FABRICADA. Item TT3 'o dia acusa -- o que a celula julgou?' em escala/services/furos_diarios.py:158 (`def ranking_colabs`, registro core/juizes.py:426, 'deveria ler classificar_falta / veredito'). Lido ao vivo: o sitio NAO re-deriva o dia -- ele so agrega FuroDiario (Sum marcos_previstos/marcos_furados/orfas, Count dias/dias_furo, Max data) e FuroDiario tem escritor unico = ponto/portas/celula.py::gravar_furos, alimentado por apurar_colab, que le a CELULA (leitor_celula.grade_da_celula, T4.2-LEITOR). Ou seja: ja le a lampada, materializada. Nenhum dos dois juizes indicados cabe mecanicamente: (a) servico_jornada.py::classificar_falta devolve (tipo_batida, motivo, verbo) da batida que falta em UM dia -- nao da marcos previstos/furados; chama-lo por colab-dia na tela seria re-julgar na leitura (o que o TABULEIRO 4a proibe: consumidor le a lampada, nunca re-julga) e ~750 x 30 chamadas por pagina; (b) CelulaDia.veredito nao e lido nem por FuroDiario nem por apurar_colab -- filtrar o 'quem cobrar' pelo veredito (ex.: descontar dia 'concorde'/'justificado') muda o NUMERO que o admin ve e quem entra na lista, e isso e regra de negocio nao escrita. Qualquer selo novo para 'trocar a fonte' ficaria verde na arvore original e na curada (nada a trocar), ou mudaria o numero sem lastro. Tres saidas, todas suas: (1) declarar que FuroDiario (escritor unico = porta da celula) E a leitura da lampada para contagem de furo e tirar a linha do registro (contador de core/tests/test_contract_juiz_tela.py anda -1); (2) o 'quem cobrar' passa a descontar dias por CelulaDia.veredito -- fatia com DIFF antes, NAO MEDIDO quantas linhas mudam (FONTE FuroDiario x CelulaDia.veredito na sombra / UNIDADE colab-dia / UNIVERSO competencia corrente, 3 CNPJs / EXCLUSOES isentos e dias nao-trabalho, que FuroDiario ja exclui; consulta na sombra nao autorizada nesta corrida); (3) manter como pendente. Nao toquei core/juizes.py, o contador nem o sitio. | 22/09 00:00 | 23 h | 1 sitio (registro TT3 ranking_colabs; 2 consumidores: relatorios/views.py:1435, relatorios/views_pend.py:25) | `corte Ronald: FuroDiario ja e a lampada, tirar ranking_colabs do registro (ou: filtrar por veredito com DIFF / manter)` |
| 70 | corte | FABRICANTE 22/09, NAO FABRICADA (re-sorteio). O item ja espera corte em corte-fila-por-causa-particionar-tt3, e o codigo vivo nao mudou desde entao: particionar pergunta 'bateu no dia PREVISTO VIZINHO?' (Batida crua + parear_turnos, CelulaDia trabalha=True), que nao e a pergunta do juiz declarado (classificar_falta = 'qual batida falta NESTE dia'); o motivo cita tres fontes (turnos_do_colab + fatos_do_dia + CelulaDia.veredito) e cada uma move chamados entre gavetas -- redefinir gaveta e regra de negocio nao escrita. Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:408-409, PENDENTES['tela'], _TT3, sitio chamados/services/fila_por_causa.py 'def particionar(base=None):'); 0 curados. FONTE: fila_por_causa.py:139-270 e core/juizes.py:344-356,408-409 relidos ao vivo + esta lista (id corte-fila-por-causa-particionar-tt3). UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma outra linha julgada. | `corte Ronald: o fabricante pula item que ja tem corte aberto em PENDENTES_RONALD.json? (particionar _TT3 foi sorteado 2x em 22/09)` |
| 71 | corte | FABRICANTE 22/09, NAO FABRICADA (re-sorteio). O item ja espera corte em corte-lavrar-badge-navbar-tt2-sem-sitio, e o codigo vivo nao mudou: linhas 63-65 so consomem a particao (particionar -> toques/bloqueios_para_fechar) e lavram inteiros; nao ha pareamento de batida para trocar por juiz. A divida real mora em fila_por_causa.particionar (corte-fila-por-causa-particionar-tt3). Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:420, PENDENTES['tela'], _TT2, sitio core/management/commands/lavrar_badge_navbar.py '_p = particionar()'); 0 curados. FONTE: lavrar_badge_navbar.py:58-77 e core/juizes.py:420-421 relidos ao vivo + esta lista (id corte-lavrar-badge-navbar-tt2-sem-sitio). UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma outra linha julgada. | `corte Ronald: a linha '_p = particionar()' do lavrar_badge_navbar sai do registro como consumidor sem sitio? (sorteada 2x em 22/09)` |
| 72 | corte | FABRICANTE 22/09, NAO FABRICADA (re-sorteio). O item ja espera corte em corte-tt3-ranking-colabs-furodiario, e o codigo vivo nao mudou: ranking_colabs so agrega FuroDiario (escritor unico = ponto/portas/celula.py::gravar_furos, alimentado pela ata da celula via grade_da_celula) -- ja le a lampada. classificar_falta responde outra pergunta (qual batida falta em UM dia) e filtrar por CelulaDia.veredito muda o numero sem regra escrita. Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:422, PENDENTES['tela'], _TT3, sitio escala/services/furos_diarios.py:158 'def ranking_colabs'); 0 curados. FONTE: furos_diarios.py:158-225, servico_jornada.py:137-197 e core/juizes.py:422-423 relidos ao vivo + esta lista (id corte-tt3-ranking-colabs-furodiario). UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma outra linha julgada. | `corte Ronald: FuroDiario ja e a lampada -- ranking_colabs sai do registro? (sorteado 2x em 22/09)` |
| 73 | corte | A fatia fez o 'quem cobrar' perguntar a EscalaColaborador.eh_dia_trabalho antes de rotular 'sem dia previsto': None em algum dia -> 'limbo' (o proprio juiz escreve 'o limbo cobra'); folga em todos -> 'sem_dia'. Sobra o terceiro veredito: o juiz diz TRABALHO em algum dia da janela e mesmo assim nao ha FuroDiario (celula sem marcos -> prev 0, ou cron de apuracao atrasado). Esse colab segue rotulado 'sem dia de trabalho previsto', que o juiz contradiz. Classe propria e chave nova em comp -> texto novo no ranking_furos.html (front, fora da corrida). | 22/09 00:00 | 23 h | Residuo da fatia SEM-DIA-PELO-JUIZ (.esteira/semdia_juiz), sitio escala/services/furos_diarios.py::universo_empresa, ramo 'sem_dia'. FONTE: furos_diarios.py:228-300 e escala/models.py:857-882 relidos ao vivo. UNIDADE: colaborador ativo. UNIVERSO: ativos da empresa sem FuroDiario na janela, nao isentos, com escala vigente e sem rotulo do termometro. EXCLUSOES: nenhuma. NAO MEDIDO: a corrida do fabricante nao executa shell/Django, entao quantos caem hoje neste balde e numero a tirar (so leitura, emp 2/3/4). | `corte Ronald: no quem cobrar, ativo com dia de TRABALHO pelo juiz e sem apuracao vira que classe? (hoje fica 'sem dia previsto', que e falso)` |
| 74 | corte | FABRICANTE 22/09, NAO FABRICADA. estado_do_dia JA le a lampada: recebe o CelulaDia.veredito lavrado pelo cartorio (via grade_da_celula, d['veredito']) e o faz sobrepor a grade pela tabela _VER_ESTADO (HX-RELATORIO-SEM-VEREDITO, 25/08). O juiz declarado, classificar_falta, responde OUTRA pergunta -- (tipo_batida, motivo, verbo) da UMA batida que falta num dia, com I/O por colab/dia -- e nao devolve estado ok/parcial/furo nem a lista de marcos furados que alimenta 'horarios furados'. Trocar exigiria decidir: (a) a tabela _VER_ESTADO e sua excecao ('ok' nao apaga furo da grade salvo veredito 'trabalhou') sao regra propria ou a traducao oficial veredito->estado?; (b) 'sem_marcos' e 'parcial' vem de onde se nao da grade? Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | Item do registro relatorios/furos.py#31 (core/juizes.py:447-448, pergunta _TT3, autoridade declarada escala/servico_jornada.py::classificar_falta em juizes.py:371). FONTE: relatorios/furos.py:24-64 e 120-170, servico_jornada.py:137-217 relidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma outra linha julgada. NAO MEDIDO: quantos dias mudariam de estado. | `corte Ronald: estado_do_dia ja le CelulaDia.veredito -- a tabela _VER_ESTADO vira a traducao oficial (sai do registro) ou o juiz de _TT3 precisa devolver estado do dia?` |
| 75 | corte | FABRICANTE 22/09, NAO FABRICADA. escalas_em_limbo JA pergunta ao juiz: o criterio de limbo e ec.eh_dia_trabalho(hoje) e (hoje+1) serem None (furos.py:81-82). A segunda metade do motivo, janela_atual, CONTRADIZ a lei: a 'foto FolgaDia do mes' e por MES CIVIL no proprio juiz (eh_dia_trabalho_calculado, _tem_foto_mes, models.py:964; defeito_folga_nao_definida, models.py:927) -- trocar a checagem para a competencia 21->20 faria o diagnostico divergir da lei. O que sobra de re-derivacao sao as 'pecas' do diagnostico (folga_dia_semana vazio + sem foto do mes), que duplicam defeito_folga_nao_definida; mas ali e' diagnostico de cadastro, nao 'o que a celula julgou', e a pergunta _TT3 nao cobre o sitio. Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | Item do registro relatorios/furos.py#32 (core/juizes.py:449-450, pergunta _TT3, 'deveria ler eh_dia_trabalho / janela_atual'). FONTE: relatorios/furos.py:67-119 e escala/models.py:919-990 relidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma outra linha julgada. NAO MEDIDO: quantos vinculos mudariam de limbo. | `corte Ronald: escalas_em_limbo ja le eh_dia_trabalho e a foto e por mes civil -- tiro relatorios/furos.py#32 do registro _TT3, ou as 'pecas' devem passar a perguntar a defeito_folga_nao_definida?` |
| 76 | corte | FABRICANTE 22/09, NAO FABRICADA. O sitio 'resp = _pdf_colaboradores(headers, rows)' nao julga o dia: so SERIALIZA em PDF as mesmas `linhas` que a tela ranking_furos renderiza, vindas de escala/services/furos_diarios.py::ranking_colabs + universo_empresa (agregado FuroDiario) -- e ranking_colabs ja e item proprio do registro (_TT3, core/juizes.py:437/439). A coluna Folha do PDF sai do bloco _cmap, tambem item proprio (_TT6, core/juizes.py:453). Curar aqui seria re-derivar no PDF o que a tela e o agregado dizem, criando uma segunda fonte; a cura real mora a montante. O 'idem' aponta para estado_do_dia/escalas_em_limbo, ja parados em corte (entradas acima). Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | Item do registro relatorios/views.py#33 (core/juizes.py:451-452, pergunta _TT3, 'tudo acima -- deveria ler idem'). FONTE: relatorios/views.py:1413-1474 relido ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma outra linha julgada. NAO MEDIDO: nada contado. | `corte Ronald: o PDF do quem-cobrar so imprime as linhas de ranking_colabs -- tiro relatorios/views.py#33 do registro _TT3 (fica coberto pelos itens de ranking_colabs e _cmap), ou mantenho ate eles curarem?` |
| 77 | corte | FABRICANTE 22/09, NAO FABRICADA. classificar_export(empresa, mes, ano) itera SO FechamentoMensal da competencia (export.py:379) e le o SNAPSHOT (fech.turnos_abertos/inconsistencias). A tela ranking_furos abre por padrao na competencia CORRENTE (ref=hoje, views.py:1414), onde ainda nao ha FechamentoMensal, e por isso foi escrita de proposito 'Sem FechamentoMensal' (comentario views.py:1447) lendo o agregado vivo FuroDiario. Trocar mecanicamente apagaria a coluna Folha de todo mundo na competencia aberta. Alem disso as portas divergem: o juiz tem ferias_com_batida e nao_homologado (a tela nao), usa data_demissao (a tela usa situacao), e nao tem ramo 'isento (Art.62)' nem 'sem apuracao'. Escolher o que a coluna diz quando nao ha fechamento (juiz so em competencia encerrada? previa do juiz sobre o vivo? rotular 'previa -- nao e o veredito do TXT'?) e decisao de negocio, e a previa tocaria folha/ (zona de dinheiro, janela de fechamento aberta ate o export 09/2026). Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | Item do registro relatorios/views.py#34 (core/juizes.py:453-454, pergunta _TT6, coluna Folha APTO/retido -> folha/export.py::classificar_export). FONTE: relatorios/views.py:1445-1466 e folha/export.py:371-440, relidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nenhuma outra linha julgada. NAO MEDIDO: nada contado. | `corte Ronald: a coluna Folha do quem-cobrar -- (a) chama classificar_export so quando a competencia tem FechamentoMensal e na aberta rotula 'previa (FuroDiario)'; (b) nasce uma previa do juiz sobre o vivo em folha/export.py (depois da janela); ou (c) reclassifico o item como 'previa, sem juiz'?` |
| 78 | corte | FABRICANTE 22/09, NAO FABRICADA. Item TT6 'o colaborador entra na folha (e por que nao)?' em relatorios/views_pend.py:46 (`def pendencias_cadastro_view`, registro core/juizes.py:455, 'natureza e contagem -- classe C; deveria ler classificar_falta / classificar_export'). Lido ao vivo: a view NAO deriva nada -- chama relatorios/pendencias.py::pendencias_cadastro sobre as linhas de ranking_colabs + universo_empresa (views_pend.py:24-26), e a 'natureza' sai por substring de `classe_txt` que a linha JA carrega (pendencias.py:17-33: LIMBO, CICLO_ERRADO, FASE_INVERTIDA, HORARIO_DESLOCADO, SEM_ESCALA, SEM_DIA, ISENTO...); a contagem e Counter sobre isso (views_pend.py:42). A pergunta que a tela responde e 'por que o sistema NAO CONSEGUE APURAR este colab (defeito de cadastro)', nao 'entra no TXT'. Nenhum dos juizes indicados responde isso: (a) servico_jornada.py::classificar_falta devolve (tipo_batida, motivo, verbo) de UMA batida faltante num dia -- nao diz defeito de cadastro; (b) folha/export.py:371 classificar_export(empresa, mes, ano) itera SO FechamentoMensal (rescisao/sem_codigo_dominio/ferias_com_batida/furo_espelho/nao_homologado) e a tela abre na competencia CORRENTE (janela_atual, views_pend.py:24), sem fechamento -- trocar esvaziaria a lista, e os motivos sao outro vocabulario. Mesmo desenho de dois cortes ja abertos sobre a MESMA fonte: corte-tt3-ranking-colabs-furodiario (ranking_colabs) e o da coluna Folha do quem-cobrar (classificar_export na competencia aberta). Selo novo ficaria verde nas duas arvores (nada a trocar) ou mudaria a tela sem lastro. Saidas, todas suas: (1) reclassificar a linha: a pergunta e 'defeito de cadastro que impede apurar', cujo escritor e universo_empresa (furos_diarios.py:~280-300) -- tirar de TT6 (contador de core/tests/test_contract_juiz_tela.py anda -1) ou mover para o juiz/pendente de escala; (2) decidir junto com o corte da coluna Folha (previa do juiz sobre o vivo, depois da janela); (3) manter pendente. Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | 1 sitio (registro TT6 views_pend; 0 juizes que respondem a pergunta da tela) | `corte Ronald: pendencias de cadastro sai de TT6 (pergunta e 'defeito de cadastro', fonte universo_empresa) -- ou: decide junto da coluna Folha / manter` |
| 79 | corte | FABRICANTE 22/09, NAO FABRICADA. Item TT3 'o dia acusa (falta, furo, ok) -- o que a celula julgou?' em relatorios/furos.py:32 (`def estado_do_dia`, registro core/juizes.py:445-446, 'deveria ler CelulaDia.veredito / classificar_falta'). Lido ao vivo: o sitio JA le o veredito lavrado (d['veredito'] vem de grade_da_celula, furos.py:132,151) e o traduz por uma tabela local _VER_ESTADO (furos.py:24-29); so cai na re-derivacao pelo `missing` da grade (classificar_dia) quando o veredito e None, 'concorde', 'coberta' ou esta FORA da tabela -- 'fato_sem_previsao' e 'fato_em_ausencia' (escala/models.py:1323-1330) nao estao nela, e fato_em_ausencia esta em VEREDITOS_ACUSAM (models.py:1340) mas a tela o mostra pela grade. A guarda de furos.py:42 e morta (_v=='ok' so quando veredito=='trabalhou', que ela mesma exclui). Por que nao e troca mecanica: (1) classificar_falta(colab, dia) responde 'qual batida falta' (tipo, motivo, verbo), com 3-5 queries por chamada; dataset_furos varre ~750 colabs x 30 dias = ~22 mil chamadas por render, e nao devolve a LISTA de marcos furados que a tela imprime (`furos`, horarios) nem ok/parcial/furo. (2) Fazer o veredito soberano exige decidir o que 'concorde' e 'coberta' pintam (ok? folga? ou segue a grade?), onde vao fato_sem_previsao/fato_em_ausencia e nunca_bateu x furo -- isso muda cor e nfuro (ordem dos cards, filtro so_com_furo) no relatorio de auditoria. Decisao de negocio nao escrita. Saidas: (a) veredito soberano: tabela FECHADA do vocabulario inteiro (VEREDITOS_CARTORIO+ADMIN) -> estado, concorde=ok e coberta=folga, grade so para `fal` (horarios) e para veredito None -- ai fabrico com selo por veredito, e a tabela deveria morar ao lado de VEREDITOS_ACUSAM, nao em relatorios/; (b) reclassificar: declarar CelulaDia.veredito (+VEREDITOS_ACUSAM) como a face em LOTE de TT3, ja que classificar_falta por dia nao escala em relatorio de frota; (c) manter. Nao toquei app/ fora desta linha. | 22/09 00:00 | 23 h | 1 sitio (registro TT3 relatorios/furos.py::estado_do_dia). FONTE: relatorios/furos.py:24-67,121-172, escala/models.py:1221-1242,1320-1345, escala/servico_jornada.py:137-217, core/juizes.py:359,371,445, lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: a linha irma escalas_em_limbo (juizes.py:447) e furos_por_dia/dias_100_em_branco (mesmo arquivo, classificar_dia sem veredito) nao foram julgadas. NAO MEDIDO: quantos colab-dia mudariam de cor sob (a) -- FONTE dataset_furos x variante na sombra / UNIDADE colab-dia / UNIVERSO competencia corrente, 3 CNPJs / EXCLUSOES fora_vinculo, sem_celula, sem_ancora. | `corte Ronald: no relatorio de furos o veredito da celula manda sozinho (concorde=ok, coberta=folga, tabela do vocabulario inteiro ao lado de VEREDITOS_ACUSAM) -- ou: declarar o veredito como face em lote de TT3 / manter` |
| 80 | corte | Item relatorios/views.py#32 (TT3, 'resp = _pdf_colaboradores(headers, rows)', 'tudo acima -- deveria ler idem') NAO fabricado. Lido ao vivo (relatorios/views.py:1468-1473): o ramo ?pdf=1 NAO julga o dia -- so formata as MESMAS `linhas` que a tela HTML renderiza, vindas de escala/services/furos_diarios.py::ranking_colabs (agregado FuroDiario) + universo_empresa. Nao ha re-derivacao local para trocar por pergunta ao juiz: a cura mora a montante, e o montante ja esta registrado a parte (juizes.py: TT3 ranking_colabs e TT3 relatorios/furos.py::estado_do_dia, este com corte aberto 'veredito da celula manda sozinho / face em lote / manter'). Curar aqui seria fatia vazia ou duplicata da montante. | 22/09 00:00 | 23 h | 1 sitio (registro TT3 relatorios/views.py 'resp = _pdf_colaboradores'). FONTE: relatorios/views.py:1395-1473, escala/services/furos_diarios.py:158-203, core/juizes.py:437,445,449, lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nao julguei ranking_colabs nem a coluna Folha (_TT6, juizes.py:451). | `corte Ronald: tirar a linha do PDF do registro (e casca de apresentacao da tela, herda a cura de ranking_colabs) -- ou reclassificar como dependente do item ranking_colabs / manter` |
| 81 | corte | Item inteligencia/calculadores.py#47 (TT7, "@metrica('prontidao_folha')", 'qual competencia encerrada -- deveria ler janela_anterior (K5)') NAO fabricado: o sitio JA ESTA CURADO. Lido ao vivo (inteligencia/calculadores.py:190-198): prontidao_folha importa e chama ponto.janelas.janela_anterior(data_fim, empresa) -- cura da fatia FECH-ENCERRADA (21/09, TICKETS.md:825, FEITO). Aquela fatia tirou do registro a linha ancorada em 'if _fim < data_fim:' (.esteira/f_encerrada/construir.py:136), mas a linha irma ancorada em "@metrica('prontidao_folha')" (core/juizes.py:479) ficou: registro orfao que infla o pendente em 1. Fatia de cura seria vazia (selo nao teria como ficar VERMELHO na arvore original). | 22/09 00:00 | 23 h | 1 sitio (registro TT7 core/juizes.py:479). FONTE: inteligencia/calculadores.py:175-229, ponto/janelas.py:66-72, .esteira/f_encerrada/construir.py, lidos ao vivo. UNIDADE: linha de registro. UNIVERSO: so este item. EXCLUSOES: nao conferi as outras linhas TT7 de fechamento. | `corte Ronald: tirar a linha orfa de core/juizes.py:479 e decrementar o contador de test_contract_juiz_tela (fatia de limpeza de registro, sem cura) -- ou manter` |
| 82 | corte | Item ponto/services/fechamento_lista.py#48 (TT6, 'if fech and empresa_id:', 'entra na folha e motivo -- deveria ler classificar_export') NAO fabricado: a cura exige decisao de negocio + front. Lido ao vivo: (1) fechamento_lista.py:59-60 deixa 'nao_homologado' FORA DE PROPOSITO ('depende de rubrica por evento e nao esta materializado; a tela declara a conferir') e :56-58 promete 'zero grade por colab' -- folha/export.py:371-431 classificar_export roda dias_ferias_com_batida + motivos_retencao_espelho(fech=) + codigos_homologados, o custo que o 31/07 PERF tirou; (2) o vocabulario difere ('rescisao'/'sem_codigo' x 'rescisao_modulo_proprio'/'sem_codigo_dominio', + ferias_com_batida/nao_homologado que a tela nao conhece): templates/ponto/_fech_linha.html:11 cai no else e rotularia ferias_com_batida como 'rescisao' -- cura correta toca template (front, proibido na corrida); (3) universo difere: classificar_export e por empresa (todo FechamentoMensal), a lista e por qs_colab; (4) o mesmo sitio ja tem linha irma K2 em core/juizes.py:335 ('tique copia a ordem de classificar_export sem ferias_com_batida nem nao_homologado'). | 22/09 00:00 | 23 h | 1 sitio, 2 linhas de registro (core/juizes.py:335 K2 e :481 TT6). FONTE: ponto/services/fechamento_lista.py:47-73, folha/export.py:371-431, templates/ponto/_fech_linha.html:11, lidos ao vivo. UNIDADE: sitio. UNIVERSO: so este item. EXCLUSOES: nao medi o custo de classificar_export na J.A. | `corte Ronald: o tique 'entra na folha' da lista do fechamento passa a ler classificar_export (aceitando o custo de ferias_com_batida/nao_homologado por pagina e ajustando _fech_linha.html com smoke) -- ou declara o tique como veredito PARCIAL materializado e tira as duas linhas do registro` |
| 83 | corte | Item colaboradores/services/calendario.py#66 (TT6 'o colaborador entra na folha?', 'def status_do_dia(colaborador, data):', 'status e contagem -- deveria ler idem') NAO fabricado: a pergunta registrada nao e a que o sitio responde. Lido ao vivo (calendario.py:168-188): status_do_dia e contar_por_status NAO decidem entrada na folha -- devolvem o status do DIA do calendario, e desde CALENDARIO-UM-JUIZ (16/09) ja sao portas curtas que montam contexto_calendario e leem o que _classificar pintou (calendario.py:89-100 declara-se o juiz unico da tela). Trocar por folha/export.py::classificar_export (autoridade de TT6, core/juizes.py:374) seria trocar de pergunta, nao de juiz; e 'idem' aponta para a linha anterior (api/views.py, realizado_do_dia + veredito), outra familia (TT3/TT4). A pergunta de verdade ('o dia acusa?') ja tem linha propria: TT3 'status = _classificar({' (core/juizes.py:519) -- curar la cura este sitio por heranca. | 22/09 00:00 | 23 h | 1 sitio (registro TT6 core/juizes.py:517). FONTE: colaboradores/services/calendario.py:89-188, core/juizes.py:362,374,517-522, lidos ao vivo. UNIDADE: linha de registro. UNIVERSO: so este item. EXCLUSOES: nao julguei a linha TT3 de _classificar nem a do rodape (:521). | `corte Ronald: tirar a linha TT6 de status_do_dia do registro (porta curta que herda a cura de _classificar, linha TT3 :519) e decrementar o contador de test_contract_juiz_tela -- ou reclassificar para TT3 como dependente de _classificar / manter` |
| 84 | corte | Item colaboradores/services/calendario.py#67 (TT3 'o dia acusa?', 'status = _classificar({', 'deveria ler fatos_do_dia + CelulaDia.veredito') NAO fabricado. Lido ao vivo: o sitio JA le as duas fontes que o motivo pede -- calendario.py:384-391 chama ponto.precedencia.fatos_do_periodo (a faixa da lei A2 entra em _classificar como 'faixa', :474/:481) e :312-318 le CelulaDia.veredito/veredito_via (entram como 'veredito'/'veredito_via', :494-495; mapeados por _VER_STATUS, :51-56 e :147-160). Isso e' CALENDARIO-UM-JUIZ (16/09). Um selo 'le fatos_do_dia + veredito' ficaria verde na arvore original e na curada: nao morde. O que _classificar ainda faz por conta propria e' COMPOR o status de tela (previsto/aberto/folga_trabalhada/sem_certidao/escala_suspeita) somando o desvio do motor (:491) e o status da grade (celstat_map, :276-306) ao veredito. A autoridade declarada de TT3 (core/juizes.py:371) e' escala/servico_jornada.py::classificar_falta, que devolve (tipo_batida, motivo, verbo) da batida que FALTA -- nao um status de dia, e 1 query por dia. Trocar _classificar por ela muda o vocabulario da tela e o que o rodape conta: decisao de negocio, nao fatia automatica. Alem disso calendario.py tem diff NAO commitado na working tree (TELA-FOLGA-ROTULO, front aguardando o seu smoke), no mesmo trecho (:124-160). | 22/09 00:00 | 23 h | 1 sitio (registro TT3 core/juizes.py:519). FONTE: colaboradores/services/calendario.py:49-165, 270-498; core/juizes.py:359,371,519-520; escala/servico_jornada.py:137-197, lidos ao vivo. UNIDADE: linha de registro. UNIVERSO: so este item. EXCLUSOES: a linha do rodape (:521) e a de HE (:523) nao foram julgadas. | `corte Ronald: (a) declarar o sitio curado por CALENDARIO-UM-JUIZ (le fatos_do_periodo + CelulaDia.veredito) e tirar a linha TT3 :519 do registro, decrementando test_contract_juiz_tela; ou (b) dizer se o status da tela deve deixar de somar desvio do motor e status da grade ao veredito -- e qual juiz responde 'previsto/aberto/folga_trabalhada' (classificar_falta nao responde)` |
| 85 | corte | Item colaboradores/services/calendario.py#68 (TT3 'o dia acusa?', ancora 'mes_prox, ano_prox = mes + 1, ano', '4 espelho/cartao: rodape (ok, faltas, irregulares...) -- deveria ler idem') NAO fabricado. Lido ao vivo: o rodape (calendario.py:573-592, dict `resumo`) nao julga nada -- so conta d['status'] dos dias da tela, e esse status sai de _classificar, o mesmo juiz que pinta o dia (L1: 'o rodape conta o que a tela mostra', docstring de contar_por_status :180-188). Fazer o rodape ler classificar_falta direto QUEBRA a L1 (numero do rodape diverge do dia pintado). A cura, se houver, mora em _classificar -- que ja tem item proprio (TT3 :519) e corte pendente (corte-calendario-classificar-ja-le-a2-e-veredito). Um selo so para o rodape ficaria verde nas duas arvores: nao morde. A ancora do registro ('mes_prox...') e' a linha de navegacao, nao o rodape. calendario.py tem ainda diff nao commitado na working tree. | 22/09 00:00 | 23 h | 1 sitio (registro TT3 core/juizes.py:521). FONTE: colaboradores/services/calendario.py:89-188, 562-592; core/juizes.py:359,371,517-522, lidos ao vivo. UNIDADE: linha de registro. UNIVERSO: so este item. EXCLUSOES: a linha de HE (:523) nao foi julgada. | `corte Ronald: tirar a linha TT3 do rodape (:521) do registro junto com a de _classificar (:519) quando decidir aquele corte -- o rodape herda a cura dela por L1 -- decrementando test_contract_juiz_tela; ou manter como dependente` |
| 86 | corte | Item colaboradores/services/calendario.py#69 (TT4 'quantos minutos o dia realizou?', ancora 'if resultado.horas_extra_50 > 0:', '4 espelho/cartao: selos de extra 50/100, atraso e antecipada -- deveria ler MotorBase.aplicar_tolerancia via o motor CCT / realizado_do_dia') NAO fabricado. Lido ao vivo: os selos (calendario.py:516-525) ja NAO re-derivam -- leem o resultado de motor.calcular_mes (:451), que aplica aplicar_tolerancia por dentro, e o selo HE ja passa por he_contabilizavel (TOLERANCIA_HE_MIN_DIA do motor). O juiz declarado da pergunta, ponto/turnos.py::realizado_do_dia, devolve RealizadoDoDia(minutos, sem_turno, aberto, longo) (turnos.py:312): NAO separa extra 50 x 100, nem atraso, nem saida antecipada -- trocar o selo por ele apaga os 4 badges. O que sobra da cura e 'via o motor CCT' = trocar get_motor por get_motor_cct, que ja e item proprio (core/juizes.py:314, _F3, 'tela mostra simples onde a folha dobra') e e regra de dinheiro na tela (dobra de feriado): corte, nao fatia estrutural. | 22/09 00:00 | 23 h | 1 sitio (registro TT4 core/juizes.py:523). FONTE: colaboradores/services/calendario.py:438-525; ponto/turnos.py:312,349-405; core/juizes.py:147,158,314,360,372,523, lidos ao vivo. UNIDADE: linha de registro. UNIVERSO: so este item. EXCLUSOES: o item _F3 :314 nao foi julgado. | `corte Ronald: (a) declarar que a pergunta destes selos NAO e TT4 (e 'quanto de extra/atraso o motor apura?', juiz = o motor) e tirar a linha :523, decrementando test_contract_juiz_tela; ou (b) curar junto com o _F3 :314 (get_motor -> get_motor_cct no calendario), uma fatia so, depois da janela de fechamento` |
| 87 | corte | Item colaboradores/services/detalhe.py#70 (TT2 'quais sao os turnos do colaborador (pares de batida)?', ancora 'total_batidas = Batida.objects.filter(colaborador=colaborador).count()', registro core/juizes.py:525-526, motivo '4 espelho/cartao: total de batidas e com GPS -- classe B; deveria ler nenhum') NAO fabricado. O proprio registro diz 'deveria ler nenhum': nao ha autoridade para onde trocar, e juiz novo e corte seu. Lido ao vivo (detalhe.py:45-49): o sitio conta TODAS as batidas cruas da vida do colaborador e quantas tem latitude/longitude, para o resumo de GPS do detalhe -- nao pareia batida nem monta turno; turnos_do_colab/parear_turnos nao respondem 'quantas batidas tem GPS'. Tambem nao e apuracao (e EXIBICAO de marcacao, que por lei le o cru -- Portaria 671, CLAUDE.md 4), entao nem batidas_apuraveis se impoe sem decidir se retratada entra no total. Nao toquei core/juizes.py, o contador nem .esteira. | 22/09 00:00 | 23 h | 1 sitio (registro TT2 core/juizes.py:525). FONTE: colaboradores/services/detalhe.py:45-49; core/juizes.py:358,525-526, lidos ao vivo. UNIDADE: linha de registro. UNIVERSO: so este item. EXCLUSOES: nenhuma. | `corte Ronald: (a) tirar a linha :525 de TT2 e marca-la SEM_JUIZ (contagem de exibicao, le o cru por lei), decrementando test_contract_juiz_tela; ou (b) decidir se o total do detalhe exclui retratada e, se sim, apontar para ponto/turnos.py::batidas_apuraveis -- ai vira fatia` |
| 88 | corte | Item colaboradores/services/drawer.py#71 (TT3 'o dia acusa (falta, furo, ok) -- o que a celula julgou?', ancora 'cal = contexto_calendario(colab, ...)', registro core/juizes.py:527, motivo '4 espelho/cartao: calendario embutido -- classe C; deveria ler idem') NAO fabricado. Lido ao vivo (drawer.py:98-119): o drawer nao julga dia nenhum -- so escolhe a janela (janela_atual + piso_visual) e CONSOME contexto_calendario, o MESMO componente do perfil (unificacao 15/08). A re-derivacao mora a montante, em colaboradores/services/calendario.py:480 ('status = _classificar({'), que e item proprio (core/juizes.py:519). Nao ha troca a fazer no drawer: curar aqui = curar o calendario, que e outra fatia; tirar a linha do drawer sem curar o calendario apagaria da lista um leitor que ainda mostra o julgamento errado. Alem disso o 'idem' aponta para 'fatos_do_dia + CelulaDia.veredito' (motivo da :519), que NAO e o juiz declarado de TT3 (escala/servico_jornada.py::classificar_falta, juizes.py:371) -- divergencia de autoridade que so voce desfaz. Nao toquei core/juizes.py, o contador nem .esteira. | 22/09 00:00 | 23 h | 1 sitio (registro TT3 core/juizes.py:527). FONTE: colaboradores/services/drawer.py:98-119; colaboradores/services/calendario.py:89,480; core/juizes.py:359,371,519,527, lidos ao vivo. UNIDADE: linha de registro. UNIVERSO: so este item. EXCLUSOES: o item :519 (calendario) nao foi julgado. | `corte Ronald: (a) declarar o drawer CONSUMIDOR do calendario e tirar a linha :527 junto com a cura da :519 (decrementando test_contract_juiz_tela); e (b) dizer qual e o juiz do status do dia no calendario -- classificar_falta (o declarado em TT3) ou fatos_do_dia + CelulaDia.veredito (o que o motivo da :519 pede)` |
| 89 | aval | APLICADO, REVISE: fatia .esteira/dia_util_feriado [DIA-UTIL-PELO-FERIADO]. _F9 'o que e dia util?' esta em SEM_JUIZ; escolhi o juiz mais proximo que ja existe, core/feriados.py::feriados_de. colaboradores/models.py::dias_uteis_do_mes (custo de VT na tela de linhas de transporte e nos dois paineis de inteligencia) passa de seg-sex cru para seg-sex MENOS feriado. Sem municipio = so nacional (o que o juiz declara); nenhum chamador passa municipio ainda. Efeito: set/2026 cai de 22 para 21 dias (07/09), e o custo de VT exibido cai junto. Nao e folha: nada disso entra em TXT nem holerite. _F9 segue em SEM_JUIZ (o escalonamento de supervisao ainda responde sozinho). | 22/09 00:00 | 23 h |  | `` |
| 90 | aval | APLICADO, REVISE: fatia .esteira/fech_tique_juiz [FECH-TIQUE-PELO-JUIZ]. O tique ENTRA NA FOLHA da lista do fechamento (ponto/services/fechamento_lista.py) passa a ser classificar_export, 1 chamada por lista com empresa escolhida. Duas escolhas minhas: (1) ferias_com_batida e nao_homologado nao tem pilula no template (front, nao toquei): saem com a pilula 'espelho' e o texto do juiz no title; o motivo cru fica em r.folha.motivo_juiz para a fatia de TELA (com smoke) dar a pilula certa. (2) CUSTO NAO MEDIDO (a corrida nao tem docker/python): o juiz gasta ~2 queries/colab no espelho (celula ao vivo em vez de fech.motivos_espelho materializado) e ~3 na conferencia de catalogo quando ha CatalogoDominio, e a lista e reconstruida em cada lote de 100. Estimativa ~1-2s por pagina/lote na J.A, mesma ordem da prontidao. Medir na sombra: classificar_export(emp2, 9, 2026) cronometrado. | 22/09 00:00 | 23 h |  | `` |
| 91 | registro | ALVO MORTO, fatia NAO fabricada: colaboradores/painel_cell.py#0 (_TT1 'quem esta em turno agora?'). O LED ja pergunta a ponto/turnos.py::turno_aberto_de desde 187e7dfe [TELA-LED-PELO-JUIZ] (painel_cell.py:64-66, selo test_led_pelo_juiz.py). A ancora do registro (painel_cell.py:32, Batida.objects.filter ... timestamp__gte=corte) sobrou so para 'ultima_batida' e a coordenada do pino geo: e EXIBICAO de marcacao crua (Portaria 671, CLAUDE.md sec.4), nao responde turno. A linha _TT1 em core/juizes.py PENDENTES['tela'] ficou velha -- tirar dali (e contador de test_contract_juiz_tela.py -1) no proximo commit que tocar juizes.py. | 22/09 00:00 | 23 h |  | `` |
| 92 | registro | FATIA NAO FABRICADA (juiz inexistente): colaboradores/services/situacional.py#1 (_TT6 'o colaborador entra na folha?'), ancora situacional.py:136 op_justif = Justificativa.objects.filter(status='pendente').count(). O sitio NAO responde 'entra na folha': conta justificativas pendentes do tenant inteiro, e classificar_export / motivos_retencao_celula (folha/export.py:371,573) nao olham justificativa nenhuma -- trocar por eles mudaria o numero para outra pergunta. Nao existe funcao que responda 'quantas justificativas pendentes' (o mesmo filtro cru se repete em ponto/views.py:1131, core/views.py:208, chamados/views.py:590). O recorte global e DECLARADO (core/migrations/0009 help_text: 'NAO tem recorte por empresa na lei de hoje', mesmo numero em toda linha do placar). A linha esta mal classificada em _TT6. Criar o juiz (ex. ponto/services/justificativa.py::pendentes(universo=None), consumido pelos 4 sitios + lavrar_placar_situacional) e juiz NOVO = corte seu. | 22/09 00:00 | 23 h |  | `` |
| 93 | registro | FATIA FABRICADA com escolha minha (.esteira/estado_app_juiz, ESTADO-APP-PELO-JUIZ): colaboradores/services/situacional.py#2 (_TT2 'turnos do colaborador', 'deveria ler nenhum declarado'). A pergunta real do sitio e 'houve batida VIVA no periodo da tela?' (ativo x bateu e parou), nao pareamento de turno. Escolhi o juiz mais proximo que ja existe: ponto/services/adesao.py::ids_com_adesao, que ganhou ini/fim/ids opcionais (mesma lei, retratada fora; sem argumento a resposta de sempre) -- 1 query por pagina. Rejeitei turnos_do_colab/batidas_apuraveis por colaborador: N+1 no lavrar_placar_situacional */5 (~750 colabs) dentro do saas_core. NAO curado, vizinho na MESMA linha da tela: o LED verde/cinza (_led) segue lendo batida_recente cru, entao quem so tem batida retratada no periodo mostra LED verde + 'bateu e parou' -- e o pendente de juizes.py '1 painel situacional: LED / ultima batida', proxima fatia. | 22/09 00:00 | 23 h |  | `` |
| 94 | registro | FATIA FABRICADA (.esteira/painel_op_turno_juiz, PAINEL-OP-TURNO-PELO-JUIZ): core/services/painel_op.py#6 (_TT1). O painel operacional trocou turnos_em_turno_stored (TurnoMaterializado gravado, liveness propria) por turno_aberto_de por candidato (batida nao retratada nas 24 h, o recorte do /painel/tv/) -- mesma receita do /painel/tv/ e do situacional. Tres escolhas minhas: (1) CUSTO: ~3-4 queries por candidato; os selos de custo do painel NAO cobrem isso (test_selo_performance tem 3 batidas de 2 dias atras, fora das 24 h = zero candidatos; test_selo_lotes_painel nao tem batida) -- nao medido em prod, cabe medir o /painel/ depois do deploy. (2) ESCOPO: so a metade _TT1 e so o tempo de turno; a coluna 'entrada' (e pino/presenca geo) segue sendo a ultima 'E' crua das 24 h; 'ausente'/'folga' seguem por eh_dia_trabalho, e o '+ fatos_do_dia' do motivo fica para a familia celula/precedencia. (3) o fim previsto do 'aberto' e janela_turno_de(tipo da escala ATIVA, data_turno), a regua do proprio juiz -- nao o dt_fim_previsto gravado (que usa a escala vigente na data_turno). Sem prova RED/GREEN local nesta corrida (sem executor): a prova e do rodar.sh. | 22/09 00:00 | 23 h |  | `` |
| 95 | registro | FATIA FABRICADA (.esteira/prazo_vencido_juiz, PRAZO-VENCIDO-PELO-JUIZ): chamados/services/fila.py#17 (_TT8). O motivo do item dizia 'deveria ler sem juiz (sla_limite_horas)', mas a pergunta 'o prazo estourou?' TEM juiz declarado: C9 = chamados/catalogo/motor.py::prazo_estourou (core/juizes.py, familia chamado; C-MIUDOS-1). Escolhi ele. prazo_vencido da gestao da fila (ponte do copiloto) virou sum(prazo_estourou(prazo_sla, agora)) sobre os chamados com prazo_sla carimbado; muda so a borda (`<` -> `<=`: o prazo que vence no instante da consulta conta como vencido). Escolha deliberada: NAO apliquei prazo_de_cobranca_do_chamado (prazo derivado de criado_em para quem nasceu sem prazo_sla), porque isso daria 20 min a qualquer chamado legado sem prazo e mudaria o sentido de com_prazo_definido/sem_prazo_definido. Sem prova RED/GREEN local nesta corrida (sem executor): a prova e do rodar.sh. | 22/09 00:00 | 23 h |  | `` |
| 96 | registro | NAO FABRICADA: core/services/painel_op.py#5 (_TT1, ancora 'delta = agora - _tm.entrada.timestamp'). A MESMA linha do registro ja e removida pela .esteira/painel_op_turno_juiz (PAINEL-OP-TURNO-PELO-JUIZ), ESTADO=PRONTA desde 20:06 de 22/09, esperando o integrador commitar -- a cura esta la, nao na arvore. Uma segunda fatia sobre a mesma ancora de core/juizes.py quebraria o construir.py de uma das duas. SOBRA REAL (fora do registro depois dela): 'ausente' x 'folga' do painel segue por eh_dia_trabalho + batida 'E' crua; nao pergunta a ponto/precedencia.py::fatos_do_dia, entao colab de FERIAS/afastamento/atestado ou em feriado da praca, em dia que a celula da como trabalho, aparece 'ausente' no /painel/. Cura desenhada: so para o candidato (dia_trabalho and not entrada), fatos_do_dia(colab, hoje)['tipo_do_dia'] not in (None, 'trabalho', 'folga') -> 'folga' (o front nao muda); custo = ~5 queries por candidato, nao por colaborador. | 22/09 00:00 | 23 h |  | `` |
| 97 | corte | FABRICADA .esteira/dash_batidas_hoje_juiz (DASH-BATIDAS-HOJE-PELO-JUIZ), item core/views.py#10 (_TT2, 'deveria ler nenhum'). ESCOLHAS DECLARADAS: (1) 'batidas hoje' do dashboard foi lido como NUMERO OPERACIONAL (coerente com em_turno/turnos abertos ao lado, que ja excluem retratada), nao como exibicao de marcacao -- o juiz escolhido e o do universo, ponto/turnos.py::batidas_apuraveis; o feed ultimas_batidas segue CRU (Portaria 671). (2) batidas_apuraveis ganhou colab=None = mesmo universo de todos, UMA query (o loop por colaborador custaria ~750 queries por carga do dashboard); chamadores atuais passam colab e nao mudam. (3) curados os DOIS sitios do numero: dashboard e /dashboard/dados/ (este ultimo nao estava no registro, mas reescreve o mesmo numero -- curar um so seria meia-correcao). (4) A corrida nao tinha executor (sem python3/docker): esteira.sh/rodar.sh montados a mao, ancoras conferidas por grep -F -c == 1, SEM prova local RED/GREEN -- o rodar.sh prova e recusa se o RED nao ficar vermelho. ATENCAO: ponto/turnos.py estava modificado na working tree no inicio da corrida; a fatia copia o arquivo inteiro da arvore viva, entao o commit do integrador leva junto o que estiver la. | 22/09 00:00 | 23 h |  | `` |
| 98 | corte | FABRICADA .esteira/fila_vizinho_celula (FILA-VIZINHO-PELA-CELULA), item chamados/services/fila_por_causa.py#18 (_TT3, 'deveria ler turnos_do_colab + fatos_do_dia + CelulaDia.veredito'). ESCOLHAS DECLARADAS: (1) curada so a perna do VEREDITO: o dia vizinho (anterior/seguinte ao cobrado) le a lampada que o cartorio ja lavrou -- 'coberta' e 'justificado' = nao era dia de bater (o previsto vizinho passa a ser o seguinte); 'trabalhou'/'faltou' do admin vence o pareamento cru. 'cobranca_indevida', 'fato_em_ausencia' e os demais ficaram NEUTROS (seguem a batida pareada). (2) NAO se pergunta classificar_falta (juiz declarado de _TT3) nem fatos_do_dia dia a dia: a fila e LOTE (~1,4k chamados x 2 vizinhos) e ambos sao por colab-por-dia; o veredito e onde o julgamento deles ja mora. (3) O pareamento continua parear_turnos sobre batida viva (retratada fora) em lote -- turnos_do_colab e o invólucro por colab do MESMO juiz e custaria uma query por colaborador; feriado ja vem de core.feriados. (4) A linha INTEIRA sai do registro, embora o motivo citasse tres fontes. (5) Corrida sem executor: esteira.sh/rodar.sh/cadeia.sh montados a mao a partir de bin/molde_fatia, ancoras conferidas por grep -c == 1, SEM prova local RED/GREEN -- o rodar.sh prova e recusa se o RED nao ficar vermelho. | 22/09 00:00 | 23 h |  | `` |
| 99 | registro | REFABRICADA .esteira/prazo_vencido_juiz2 (PRAZO-VENCIDO-PELO-JUIZ), item chamados/services/fila.py#16 -- o mesmo alvo da .esteira/prazo_vencido_juiz, que caiu 20:13 com 'GREEN parcial vermelho' (o RED tinha mordido certo: 1 != 2). Causa lida no green_parcial.out: qs_na_fila herda prefetch_related('followups') de montar_qs_filtrado, e values_list(flat=True) com prefetch levanta AttributeError ('Cannot find followups on FakeDatetime'). Unica mudanca: .prefetch_related(None) antes do values_list, na cura e no selo (mesmo idioma de painel.py/verbos.py). Mesmas escolhas da 1a (entrada prazo-vencido-pelo-juiz-escolha): juiz C9 prazo_estourou, so prazo carimbado, legado sem prazo_sla fora. Sem prova local (corrida sem executor): a prova e do rodar.sh. | 22/09 00:00 | 23 h |  | `` |
| 100 | registro | REFABRICADA .esteira/fila_vizinho_celula2 (FILA-VIZINHO-PELA-CELULA), item chamados/services/fila_por_causa.py#17 (_TT3, 'def particionar(base=None):') -- o mesmo alvo da .esteira/fila_vizinho_celula, que caiu 20:29 com 'GREEN parcial vermelho'. Causa lida em red.out/green_parcial.out: StopIteration nas DUAS arvores (6 errors) -- o chamado do selo nao estava em gaveta nenhuma. Leitura (sem executar): ChamadoColaborador.criar dispara chamados/signals.py::_cartorio_post_save_chamado -> cartorio.julgar_celula do dia, e o selo dava celula ao dia cobrado (o teste irmao test_fila_por_causa nao da); criar batida tambem aciona o juiz do dia dela e podia reescrever o veredito do vizinho. Mudanca so no selo: particionar recebe o chamado como `base` e os vereditos das 4 celulas sao escritos por .update() (sem signal) depois de tudo nascer; + assert p['total']==1. A cura e byte a byte a da 1a; mesmas escolhas (entrada fila-vizinho-pela-celula-escolha-de-juiz). Sem prova local (corrida sem executor): a prova RED/GREEN e do rodar.sh. | 22/09 00:00 | 23 h |  | `` |
| 101 | registro | NAO FABRICADA (alvo ja em voo): item core/views.py#6 (_TT1, 'chamados_emergencia = ChamadoColaborador.objects.filter(', em_turno do dashboard) ja tem fatia .esteira/dash_em_turno_juiz (DASH-EM-TURNO-PELO-JUIZ, em_turno = len(turnos_abertos) do juiz turno_aberto_de), fatia.done ESTADO=PRONTA 20:41, 'entregue ao lote (o integrador commita)', linha 69 da logs/fila_esteira.txt. A arvore viva ainda tem em_turno = len(_em_turno_ids) e a linha no registro porque o lote nao commitou -- por isso o fabricante re-sorteou o alvo. Refabricar duplicaria a cura e colidiria no integrador. | 22/09 00:00 | 23 h |  | `` |
| 102 | registro | NAO FABRICADA (alvo morto para a pergunta do item): a linha esta sob _TT4 ('quantos minutos o dia realizou?', juiz ponto/turnos.py::realizado_do_dia), mas painel.py:376 nao calcula minuto nenhum -- e o prazo do CHAMADO, e ja pergunta a uma autoridade (chamados/services/sla.py::vencido, corte 15/09, a mesma que o cron alertar_chamados_sla le). Nao ha re-derivacao de _TT4 para trocar por realizado_do_dia. A motivacao do item ('C9 esta em SEM_JUIZ') tambem esta velha: C9 tem juiz desde C-MIUDOS-1 18/09 (motor.prazo_estourou). ACHADO REAL, que NAO curei: C9 tem DUAS LEIS VIVAS -- sla.vencido (relogio reinicia na ultima resposta do DP, limite sla_limite_horas; lida pelo painel e pelo push alertar_chamados_sla) x motor.prazo_estourou(prazo_de_cobranca_do_chamado) (prazo carimbado/derivado de criado_em; lida por escalonar_chamados_supervisao e cobranca.py:242). Trocar so o painel para prazo_estourou separaria a tela do push que ela explica (meia-correcao); unificar as duas muda o volume de push de SLA em prod. Proxima fatia sugerida: reclassificar esta linha de _TT4 para _C9 e decidir qual das duas leis e a do C9. Tambem sobra painel.py:377 (sla_alerta = 75% re-derivado fora de sla.py). NAO MEXI EM app/. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:423, _TT4, chamados/services/painel.py). FONTE: codigo VIVO (painel.py:370-377, chamados/services/sla.py, chamados/catalogo/motor.py:54, core/juizes.py:187). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nao contei leitores de sla_estourado no template. | `` |
| 103 | registro | NAO FABRICADA (alvo ja em voo): fila.py:164 ('prazo_vencido': com_prazo.filter(prazo_sla__lt=agora).count()) segue cru na arvore, mas a cura dele ja esta pronta em .esteira/prazo_vencido_juiz2 (PRAZO-VENCIDO-PELO-JUIZ, troca por motor.prazo_estourou, que e o juiz C9), LANCADA 20:38 e 'entregue ao integrador' -- so falta o commit. Fabricar de novo daria duas fatias com a mesma ancora em core/juizes.py; a segunda cairia no construir. O registro reapresentou o alvo (agora #15, antes #16/#17/#20) porque a linha so sai do juizes.py quando a fatia integra. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:413, _TT8, chamados/services/fila.py#15). FONTE: codigo VIVO (fila.py:164 ainda cru na arvore; .esteira/prazo_vencido_juiz2/esteira.out). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. | `` |
| 104 | registro | NAO FABRICADA (alvo ja em voo): a cura deste sitio ja esta pronta em .esteira/fila_vizinho_celula2 (FILA-VIZINHO-PELA-CELULA: o vizinho le CelulaDia.veredito), LANCADA 20:43 (red=failures=4, suite=2978) e 'entregue ao integrador' -- so falta o commit. Fabricar de novo daria duas fatias com a mesma ancora em core/juizes.py e em fila_por_causa.py; a segunda cairia no construir. O registro reapresentou o alvo (agora #16, antes #17/#18) porque a linha so sai do juizes.py quando a fatia integra. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:415, _TT3, chamados/services/fila_por_causa.py#16 'def particionar(base=None):'). FONTE: codigo VIVO (fila_por_causa.py:188-191 ainda le CelulaDia trabalha=True cru, sem veredito) + .esteira/fila_vizinho_celula2/{fatia.done,pronta.json,esteira.out}. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. | `` |
| 105 | aval | FABRICADA com escolha declarada (.esteira/sla_painel_juiz, SLA-PAINEL-PELO-JUIZ). O motivo do item dizia 'deveria ler nenhum (C9 esta em SEM_JUIZ)' -- velho: C9 esta declarado em JUIZES como chamados/catalogo/motor.py::prazo_estourou. Escolhi esse juiz: sla.vencido passa a responder prazo_estourou(vence_em, agora) (borda <= em vez de <; muda tambem o cron alertar_chamados_sla, que le o mesmo sla.vencido). O alerta de 75% e as horas sem resposta NAO ganharam juiz novo: sairam do loop da tela para chamados/services/sla.py (em_alerta, horas_sem_resposta, FRACAO_ALERTA=0.75), o modulo que o corte de 15/09 fez casa do prazo. A linha estava rotulada _TT4 ('quantos minutos o dia realizou?'), o que e erro de classificacao do registro -- nada a ver com minutos realizados. Fatia sem prova local (a corrida nao executa python/docker): o RED/GREEN acontece no rodar.sh. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py, _TT4, chamados/services/painel.py 'sla_estourado = _sla.vencido(c, agora_sla, followups)'). FONTE: codigo VIVO (painel.py:372-377, sla.py:33-39, motor.py:54-56). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. | `` |
| 106 | aval | FABRICADA com escolha declarada (.esteira/ranking_veredito, RANKING-ACUSA-PELO-VEREDITO), supersede corte-tt3-ranking-colabs-furodiario e corte-fabricante-resorteia-ranking-colabs-tt3 (autorizacao previa de 22/09). Juiz escolhido: CelulaDia.veredito (VEREDITOS_ACUSAM), nao servico_jornada.classificar_falta (esse responde 'qual batida falta em UM dia', outra pergunta). Leitura: fur, dias_furo e ultimo do quem cobrar contam so dia cuja celula acusa ou ainda nao foi julgada (veredito NULL segue cobrando); o dia resolvido NAO some da linha -- prev e dias ficam (o pct cai, nao sobe). nunca_bateu continua no bruto (e sobre BATIDA). ranking() (percentual por posto/praca/template/supervisor) NAO mudou: tratei como aderencia da frota, pergunta diferente. Fatia sem prova local (a corrida nao executa python/docker): o RED/GREEN acontece no rodar.sh. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:427, _TT3, escala/services/furos_diarios.py 'def ranking_colabs'). FONTE: codigo VIVO (furos_diarios.py:158-225, escala/models.py CelulaDia.VEREDITOS_ACUSAM/VEREDITOS_ADMIN, ponto/portas/celula.py::lavrar_veredito). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. NAO MEDIDO quantas linhas do quem cobrar mudam em prod (sombra nao consultada nesta corrida); recorte para medir: FONTE FuroDiario x CelulaDia.veredito / UNIDADE colab-dia com marcos_furados>0 / UNIVERSO competencia corrente, emp 2,3,4 / EXCLUSOES isentos e dia nao-trabalho (FuroDiario ja exclui). | `` |
| 107 | recusa | NAO FABRICADA -- alvo sem re-derivacao propria. O ramo PDF so formata a MESMA lista `linhas` que a tela renderiza (montada por escala/services/furos_diarios.py::ranking_colabs + universo_empresa + o bloco _cmap da coluna Folha); nao julga dia nenhum. O 'o dia acusa' dele vem inteiro de ranking_colabs, que ja tem fatia na esteira (.esteira/ranking_veredito, RANKING-ACUSA-PELO-VEREDITO, fila_esteira.txt:79), e a coluna Folha e o item irmao _TT6 (juizes.py:441). Uma cura aqui nao tem o que trocar, e um selo nao ficaria VERMELHO na arvore original (verde nas duas = nao morde). | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:439, _TT3, relatorios/views.py 'resp = _pdf_colaboradores(headers, rows)'). FONTE: codigo VIVO relatorios/views.py:1468-1473 (ranking_furos, ramo pdf=1). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. | `` |
| 108 | escolha | FABRICADA (.esteira/quem_cobrar_folha, QUEM-COBRAR-FOLHA-PELO-JUIZ). Escolha declarada: classificar_export so responde para quem TEM FechamentoMensal na competencia; a tela re-derivava uma PROJECAO (situacao/codigo/isento/FuroDiario) justamente para a competencia corrente, que ainda nao tem fechamento. Escolhi NAO projetar: sem fechamento -> folha=None, motivo 'sem fechamento' (o pill 'Projecao folha' da competencia corrente tende a 0 ate o fechamento ser calculado). Segunda escolha: o juiz roda por pagina aberta da tela (mesmo custo do painel da folha, folha/views.py:228); com competencia passada fechada ~300 fechamentos/empresa, nao medido. FATIA DE TELA DEVIDA (template proibido nesta corrida): templates/relatorios/ranking_furos.html:42 (tooltip 'PROJECAO de folha pelo agregado do periodo... estimativa' e rotulo 'Projecao folha') passa a mentir -- o numero agora e o veredito do juiz; trocar texto para 'Folha (veredito do TXT; sem fechamento = ainda nao calculado)'. Uso pratico ate la: para ver quem entra no TXT, escolher a competencia ANTERIOR no seletor (ela tem fechamento). | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:441, _TT6, relatorios/views.py "c = _cmap.get(l['chave'])"). FONTE: codigo VIVO relatorios/views.py:1445-1466 (ranking_furos, coluna Folha) e folha/export.py::classificar_export. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. | `` |
| 109 | escolha | FABRICADA (.esteira/nunca_bateu_juiz2, NUNCA-BATEU-PELO-JUIZ; a 1a corrida, .esteira/nunca_bateu_juiz, caiu em GREEN parcial vermelho 21:53 por dois selos de arvore: TESTE-SEM-RELOGIO -- o selo tinha um timezone.now() solto, agora @freeze_time -- e MAPA-VIVO acusando ponto/turnos.py, ja curado no HEAD 35f3f272). O registro pedia 'adesao.ids_com_adesao + turnos.batidas_apuraveis'; escolhi SO ids_com_adesao(ini, fim, ids): a pergunta e 'houve batida viva na janela?' (conjunto de ids), e batidas_apuraveis e por colaborador (1 query por linha) e responde 'quais batidas'; a lei de viva (retratada_em nulo) e a mesma nos dois. Janela = meia-noite local de ini ate 23:59:59.999999 local de fim (o que timestamp__date fazia com TIME_ZONE America/Sao_Paulo). Fatia sem prova local (a corrida nao executa python/docker): o RED/GREEN acontece no rodar.sh. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:429, _TT2, escala/services/furos_diarios.py 'from ponto.models import Batida as _B'). FONTE: codigo VIVO escala/services/furos_diarios.py:213-216 (ranking_colabs, coluna nunca_bateu) e ponto/services/adesao.py::ids_com_adesao. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. NAO MEDIDO em prod (a corrida nao executa python/docker); por leitura, mesmo conjunto que a query antiga. | `` |
| 110 | alvo-morto | NAO FABRICADA. A pergunta do sitio ('esta escala esta em limbo?') JA e respondida pelo juiz: relatorios/furos.py:85 chama ec.eh_dia_trabalho(hoje) e (hoje+1) e so entra quem o juiz devolve None -- nao ha re-derivacao a trocar. O resto da funcao (lista 'pecas') e DIAGNOSTICO de cadastro, nao veredito. A sugestao 'janela_atual' do registro seria ERRADA aqui: 'sem foto FolgaDia do mes' le o MES CIVIL porque a propria lei le mes civil (escala/models.py:964-966, _tem_foto_mes por data.year/data.month); trocar para competencia 21-20 faria o diagnostico divergir do juiz. A linha do registro so esta viva por inercia. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:437, _TT3, relatorios/furos.py 'def escalas_em_limbo(empresa=None):'). FONTE: codigo VIVO relatorios/furos.py:70-118 e escala/models.py:857 (EscalaColaborador.eh_dia_trabalho) / :930-967 (eh_dia_trabalho_calculado, _tem_foto_mes). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. Por leitura, nao medido. | `` |
| 111 | escolha-declarada | FABRICADA (.esteira/pend_cadastro_juiz, PEND-CADASTRO-PELO-JUIZ). O registro pedia 'classificar_falta / classificar_export'; escolhi SO classificar_export, e so a porta de CADASTRO dele (sem_codigo_dominio): quem o juiz poe fora do TXT por falta de codigo do Dominio passa a aparecer na tela com natureza SEM_CODIGO_DOMINIO (antes ficava fora do TXT e ausente da tela). As naturezas de ESCALA (LIMBO, CICLO_ERRADO, FASE_INVERTIDA...) seguem vindo do classe_txt do ranking, porque classificar_export nao conhece escala e classificar_falta julga o DIA, nao o cadastro. Rescisao, ferias com batida, furo de espelho e codigo fora do catalogo NAO entram: nao sao pendencia de cadastro (a tela irma de ESPELHO e a coluna Folha do 'quem cobrar' respondem esses). Competencia = a que termina em janela_atual(hoje).fim (mes/ano de fim); sem FechamentoMensal o juiz nao fala e a tela nao inventa. Fatia sem prova local (a corrida nao executa python/docker): o RED/GREEN acontece no rodar.sh. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:443, _TT6, relatorios/views_pend.py 'def pendencias_cadastro_view(request):'). FONTE: codigo VIVO relatorios/views_pend.py:47-55, relatorios/pendencias.py:17-30/120-132 e folha/export.py:371-440 (classificar_export). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. Por leitura, nao medido. | `` |
| 112 | alvo-morto | NAO FABRICADA (alvo ja curado). prontidao_folha ja pergunta ao juiz que o registro pede: calculadores.py:198 `_ini, _fim = janela_anterior(data_fim, empresa)` -- cura FECH-ENCERRADA (21/09), que trocou o laco de ate 3 saltos pela chamada, com selo em holerite/test_fech_encerrada_pelo_juiz.py. Nao ha re-derivacao a trocar; um selo novo ficaria verde nas duas arvores (nao morde). A linha do registro ficou viva por inercia: a FECH-ENCERRADA curou o sitio e nao tirou a linha de core/juizes.py. Achado lateral: o selo dela mora em holerite/ (fora de tests/) e testa inteligencia -- confirmar que o runner o descobre. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:469, _TT7, inteligencia/calculadores.py "@metrica('prontidao_folha')"). FONTE: codigo VIVO inteligencia/calculadores.py:190-201 e ponto/janelas.py:66-72 (janela_anterior). UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. Por leitura, nao medido. | `` |
| 113 | alvo-morto | NAO FABRICADA (alvo morto: pergunta errada e sitio ja curado). status_do_dia NAO responde 'o colaborador entra na folha?': devolve o status de UM dia do calendario, e desde CALENDARIO-UM-JUIZ (16/09) e porta curta que monta contexto_calendario de 1 dia e devolve ctx['dias'][0]['status'] -- o MESMO juiz da tela (_classificar), sem re-derivacao nenhuma. classificar_export julga colaborador x competencia a partir de FechamentoMensal (rescisao/codigo/ferias/espelho/catalogo); trocar o status de um dia por ele nao e cura, e mudanca de significado. Nenhum chamador de producao (so 3 arquivos de teste: test_calendario_um_juiz, test_a2_precedencia). A pergunta real do sitio e _TT3, que ja esta registrada na linha irma 'status = _classificar({' (juizes.py:507) -- esta linha e duplicata mal classificada. Um selo novo ficaria verde nas duas arvores. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:505, _TT6, colaboradores/services/calendario.py 'def status_do_dia(colaborador, data):'). FONTE: codigo VIVO colaboradores/services/calendario.py:168-177 e folha/export.py:371 (classificar_export); grep de status_do_dia no app. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. Por leitura, nao medido. | `` |
| 114 | alvo-morto | NAO FABRICADA (alvo morto: sitio consumidor, nao derivador). O command nao pareia batida nenhuma: chama particionar() UMA vez e repassa a mesma particao a toques(p=) e bloqueios_para_fechar(p=). badge_toques, registrados e bloqueios_para_fechar saem inteiros de fila_por_causa.py, e cada uma dessas funcoes JA tem linha propria no registro (particionar _TT3, toques _TT2, bloqueios_para_fechar _TT7) -- esta linha e duplicata. Um selo aqui ficaria verde nas duas arvores. A re-derivacao REAL esta em fila_por_causa.py:178-189: Batida.objects.filter cru + parear_turnos sem marcos/DNA (data_turno do cross-meia-noite pode cair no dia errado), em vez de turnos_do_colab. Curar la com turnos_do_colab por colab e N+1 (2-4 queries x centenas de colabs, janela desde o chamado mais antigo) no caminho do */5 que ja derrubou worker em 11/09. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:427, _TT2, core/management/commands/lavrar_badge_navbar.py '_p = particionar()'). FONTE: codigo VIVO lavrar_badge_navbar.py:58-77 e chamados/services/fila_por_causa.py:139-199; grep -E 'Batida\|parear\|turnos_do_colab\|fatos_do_dia' no command = 0 linhas. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. Por leitura, nao medido. | `` |
| 115 | alvo-em-voo | NAO FABRICADA (duplicata de uma fatia em voo). O sitio ainda le o mes civil no codigo vivo (chamados/views.py:865, criado_em__year/__month), mas a cura ja existe: .esteira/fio_msgs_comp2 [FIO-MSGS-COMPETENCIA] esta ESTADO=PRONTA, 'entregue ao lote (o integrador commita)' 22/09 22:32, com RED failures=3 na arvore anterior e suite de 2978 testes verde. Uma segunda fatia bateria nas mesmas ancoras e colidiria no lote. O registro so continua listando o item porque o integrador ainda nao fez o commit. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:423, _TT7, chamados/views.py 'contador_msgs_mes = FollowupChamado.objects.filter('). FONTE: .esteira/fio_msgs_comp2/{fatia.done,esteira.out,pronta.json}; logs/fila_esteira.txt:76. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: nenhuma. | `` |
| 116 | escolha-declarada | FABRICADA .esteira/calendario_ata_decide [CALENDARIO-ATA-DECIDE-O-DIA], sem prova RED/GREEN local (a corrida nao tem executor; rodar.sh recusa a entrega se o RED nao ficar vermelho). ESCOLHA: o juiz de tabela de TT3 e escala/servico_jornada.py::classificar_falta, que devolve a batida que falta (tipo, motivo, verbo), nao um status de dia -- nao serve de porta para a tela. Segui o MOTIVO da linha ('deveria ler fatos_do_dia + CelulaDia.veredito') e o 4a (consumidor le a lampada, nunca re-julga): o sitio ja lia as duas fontes, mas so deixava a ata ACUSAR; a absolvicao so preenchia sem_certidao/sem_dados. Resultado: dia de trabalho com batida e veredito concorde saia 'irregular' pela regra de pontas propria da tela (celstat_map) ou pelo atraso do motor. Agora, com ata lavrada, ela decide (acusando ou absolvendo); grade e motor so pintam o dia SEM ata. Isto INVERTE, para dia com ata, a postura do T4-LEI-A ('o leitor acusa mais do que absolve', 296 x 143 dia-colab na medicao de 01/09). ATENCAO: calendario.py tem na arvore o hunk de servico do TELA-FOLGA-ROTULO (fora do git, aguardando smoke); a cura copia a arvore viva, entao o commit da fatia leva esse hunk junto (so o .py do servico; os templates dele ficam de fora). | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:507, _TT3, colaboradores/services/calendario.py 'status = _classificar({'); 2a queima do item (a 1a e corte-calendario-classificar-ja-le-a2-e-veredito). FONTE: codigo VIVO calendario.py:89-165 e :263-321. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: rodape (:509) e selos HE (:511) nao tocados. Diferencial na sombra NAO medido nesta corrida (sem executor). | `` |
| 117 | escolha-declarada | FABRICADA .esteira/detalhe_total_apuravel [DETALHE-TOTAL-APURAVEL], sem prova RED/GREEN local (a corrida nao tem executor; rodar.sh recusa a entrega se o RED nao ficar vermelho). ESCOLHA: a linha dizia 'deveria ler nenhum'; o juiz mais proximo que JA existe para o universo de batidas do colaborador e ponto/turnos.py::batidas_apuraveis (retratada_em NULL). Os selos '🎯 N batidas' e '📍 N com GPS' do detalhe passam a nao contar retratada; o mapa segue cru (Portaria 671). | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py, _TT2, colaboradores/services/detalhe.py 'total_batidas = Batida.objects.filter(colaborador=colaborador).count()'). FONTE: codigo VIVO detalhe.py:45-49. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: o total irmao de colaboradores/views.py:850 (linha _TT7 propria) e o mapa/pinos com GPS (exibicao de marcacao) nao tocados. Quantas batidas retratadas o selo deixa de contar em prod NAO medido nesta corrida (sem executor). | `` |
| 118 | alvo-morto | NAO FABRICADA (alvo morto: sitio consumidor, nao derivador). A ficha nao pareia batida nem le CelulaDia: chama escala.services.propositor.propostas(hoje, colaborador_id) e COPIA campos do primeiro dict ('resolve' = len(chamados_que_morreriam)). Um selo aqui ficaria verde nas duas arvores. A re-derivacao REAL mora no propositor: Batida.objects.filter cru (retratada_em NULL) em lote + parear_turnos sem marcos/DNA e sem _marca_ecos_flush (_dias_batidos, _horario_mediano, pausa), em vez de turnos_do_colab. propositor.py nao tem linha propria no registro. Curar la com turnos_do_colab por colab e N+1 em propostas() de lote (pauta, painel) -- mesma decisao do item badge-navbar-particionar-alvo-morto-tt2. | 22/09 00:00 | 23 h | 1 linha do registro (core/juizes.py:517, _TT2, colaboradores/services/ficha.py "proposta = {'tipo': p['tipo'], 'atual': p.get('template_atual'), 'proposta': p.get('template'),"). FONTE: codigo VIVO ficha.py:162-173 e escala/services/propositor.py:79-87, :413-419, :447-452. UNIDADE: sitio. UNIVERSO: so esta linha. EXCLUSOES: a linha irma _TT5 da ficha (espelho_do_colab, :519) nao tocada. Por leitura, nao medido. | `` |
