...
- **(3a) COBRAR-DIA na esteira.** Botao "Cobrar este dia (pergunta no app)" dentro da caixa do "Resolver dia": o dia passa pelo cartorio (rejulgado com a ata de agora, emissor canonico, chamado do dia) e, se o chamado ficou sem pergunta, pela fabrica de sempre, com aviso ao colaborador; trilha "manual por <admin>". Recusa em lingua de admin: dia que nao terminou, competencia trancada ("decisao pelo DP"), veto da regua, ata sem furo. **Sobe depois da raia de dinheiro das 14:00** (o cartorio escreve ata e veredito, e o congelamento vai ate 14:00) -- ou antes, se voce liberar. Esperando o seu smoke depois de subir. *Para a admin:* "no calendario, em Resolver dia, o botao Cobrar este dia pede ao colaborador pelo app quando o sistema nao cobrou".
- **(3b) PARADO-CORTE -- lista do "Nova solicitacao" do admin:** o formulario le a `TAXONOMIA` (supervisao: Ponto, Escala, Ausencia, Posto, Conduta, Outros · dp: Holerite, Beneficios, Ferias, Documentos, LGPD, Outros · ti: Acesso, App, Equipamento · seguranca: EPI, Ocorrencia) e grava sempre o modulo "chamados"; o catalogo (`chamados/catalogo/modulos.py`) lista os 34 EMISSORES do sistema por dono, nao opcoes de abertura. Nao ha fonte unica das opcoes para "corrigir pelo catalogo" sem decidir o mapa. Faltas medidas contra o resto do sistema: areas **rh** e **hasner** (existem nas Pautas, nao no formulario); **cadastro** (dono de 7 modulos, sem area); ti e seguranca sem "Outros"; modulos de dono humano que so nascem pelo sistema: ausencia_colab, desligamento_revisao, mensagem_dp, beneficio (dp) · disputa_supervisao_manual, regularizacao_externa, agenda_dia (supervisao). Corte seu: quais destes viram opcao do formulario, e se a opcao grava o modulo do catalogo em vez de "chamados".
