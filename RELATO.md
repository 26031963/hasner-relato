...
`app/docs/RELATORIOS-PLANO.md`. Aqui ficam os quatro numeros que mudam decisao:

1. **13 dos 15 cards tiveram ZERO acesso em 8 dias** (19-26/09, a janela que o log guarda). So dois
   tiveram trafego: Cartoes em lote (44 GET + 77 POST) e Log de Auditoria (1). E o indice
   `/relatorios/` teve **50 acessos** -- as pessoas entram e nao clicam em nada.
2. **Extrato parcial custa 110-135 queries e 126-147 ms POR COLABORADOR**, porque chama
   `motor.calcular_mes` dentro de `for colab in qs_colab` (`relatorios/views.py:456` e `:489`).
   Em 5.000 pessoas: ~675 mil queries, ~12 min. E o unico card que poe o MOTOR dentro de um
   request, e nao tem NENHUM teste.
3. **Beneficios consolidados e fossil medido**: a tabela `Beneficio` tem 4 linhas no banco inteiro,
   ultima competencia 05/2026, zero em 09/2026. A tela abre vazia hoje e sempre abriu.
4. **O caderno da folha sai 99,8% sem hash CLT, e nao por recusa**: das 75 geracoes de lote (todas
   desde 17/09), **71 foram por PERIODO LIVRE**, e `folha/services.py:174` manda o periodo livre
   direto ao informacional sem nem tentar o auditavel -- 1.709 de 1.720 espelhos. Por competencia
   foram 4 lotes: 11 espelhos, 4 com hash. A cura se divide: E2 para os 7 que o auditavel
   recusou, TELA para os 1.709 que sairam por data livre.

Mapa do (D), que fecha 15=15: 3 aposentados (#2 Quem cobrar, #3 Fila do flip, #12 Beneficios) ·
4 absorvidos (#6 e #11 -> #5; #8 e #10 -> O30) · 2 receptores (#1, #5) · 1 reescrito (#7) ·
5 mantidos · +1 card novo (`pendencias/cadastro/`, que e a tela CADASTRO x REALIDADE onde o col207
espera). **O index cai de 15 para 8.**

UM ACHADO QUE CONFERI ANTES DE REPETIR, porque ele chegou com cara de incendio: "4 views pedem
`is_staff` e 3 delas nao tem `@login_required`". As tres existem
(`relatorios/views.py:1420`, `views_pend.py:75` e `:88`), **e nao sao buraco de autenticacao**:
`AnonymousUser.is_staff` e False, entao o anonimo cai no `redirect(core:dashboard)` -- fecha, nao
serve dado. O defeito real e outro e e da L-002: elas falam um segundo idioma de permissao,
`is_staff`, quando o juiz da casa e `colaboradores/mixins.py::tem_acao` com `ver_relatorios`.
Vira fatia de vocabulario, nao pressa. PROVA: `sed` nas tres views, guarda lida linha a linha.

E o `Posto.__str__` (`colaboradores/models.py:187`) explica sozinho o N+1 de TRES cards
(596 + 425 + 49 queries): ele interpola `self.empresa` e nenhum dos tres traz `posto__empresa` no
`select_related`. Uma linha de `__str__` pagando o preco em tres telas.
...
267 commits de fatia trazem o mesmo `Co-Authored-By`. O discriminador que existe de verdade e outro:
ate ser desligada em 26/09, a `cadeia.sh` emitia `TICKETS: placar e rodape (<NOME>)` logo depois de
cada fatia **dela** -- entao a fatia cujo nome aparece num desses commits e do fabricante, e as
outras sao desta sessao. COBERTURA: vale de 19/09 ao desligamento; o que veio depois e desta sessao
por construcao, porque a fabrica esta parada.

| quem | commits | % | h com commit | churn `bin/`+esteira | churn `app/` codigo | churn docs |
|---|---:|---:|---:|---:|---:|---:|
| **sessao** (fatia) | 218 | 47% | 76 | 6.721 | 30.224 | 15.123 |
| **sessao** (doc/TICKETS) | 58 | 13% | 35 | 50 | 22.547 | 67.791 |
| **integrador** (lote + assinatura) | 78 | 17% | 46 | 0 | 192 | 458 |
| **fabricante** (placar e rodape) | 57 | 12% | 34 | 0 | 0 | 318 |
| **fabricante** (fatia) | **46** | 10% | 26 | 1.146 | **7.520** | 1.575 |
| vigia | 2 | 0% | 1 | 0 | 0 | 6 |
| **TOTAL** | **459** | 100% | 99 | | | |

O numero que decide a leitura: o fabricante respondeu por **22% dos commits** e por **12,5% do churn
de codigo em `app/`** (7.520 de 60.291). E 57 dos seus 103 commits -- mais da metade -- nao eram
fatia, eram o `placar e rodape` que ele mesmo emitia atras de cada uma; foi exatamente esse par que
o item (2) da ordem desligou, e e o que permitiu medir os outros 46.

PROVA: `git log --since=2026-09-19 --numstat` classificado pelo nome em `placar e rodape (...)`;
49 nomes assinados, dos quais 46 casam com um commit `[NOME]` (os outros 3 sao texto de lote, nao fatia).
...
**POUSOU no push33.** PROVA, do log `logs/push33.log`: arsenal `Ran 8345 tests ... OK (skipped=34)`
sobre o commit `4c081b17cf`, control-plane `Ran 22 tests ... OK`, `rc_push=0`, e depois do `git fetch`
**`origin/main = 4c081b17`** com **`ahead = 0`**. Os 20 commits que estavam parados atras de
`b81544bc` desde 02:47 entraram todos de uma vez -- e os cinco smokes que barravam o push passaram
sem que uma linha de codigo de produto fosse tocada, porque nunca foi codigo: era a montagem.
...
**26/09 05:55 vigia da esteira (ALARME)** -- trava A (estrutural) vazia: nenhuma fatia viva, nova ou para relancar na fila.


**26/09 07:00 vigia da esteira (ALARME)** -- trava A (estrutural) vazia: nenhuma fatia viva, nova ou para relancar na fila.


**26/09 08:05 vigia da esteira (ALARME)** -- trava A (estrutural) vazia: nenhuma fatia viva, nova ou para relancar na fila.


**26/09 09:10 vigia da esteira (ALARME)** -- trava A (estrutural) vazia: nenhuma fatia viva, nova ou para relancar na fila.


**26/09 10:10 vigia da esteira (ALARME)** -- trava A (estrutural) vazia: nenhuma fatia viva, nova ou para relancar na fila.
...
**26/09 13:25 vigia da esteira (ALARME)** -- trava A (estrutural) vazia: nenhuma fatia viva, nova ou para relancar na fila.
