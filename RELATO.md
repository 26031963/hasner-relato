...
**22/09 23:15 vigia da esteira (ALARME)** -- trava A (estrutural) vazia: nenhuma fatia viva, nova ou para relancar na fila.


**22/09 23:40 vigia da esteira (ALARME)** -- a fatia espelho_ata_decide caiu por vermelho DELA (GREEN parcial vermelho) -- nao relanco.


**22/09 23:40 vigia da esteira** -- vigia relancou prazo_vencido_juiz2 as 23:40 (baseline divergiu: a arvore andou depois do teste da fatia). Para a admin: nada muda.


**22/09 23:45 vigia da esteira** -- vigia relancou fila_vizinho_celula2 as 23:45 (baseline divergiu: a arvore andou depois do teste da fatia). Para a admin: nada muda.


**22/09 23:50 vigia da esteira (ALARME)** -- a fatia fila_vizinho_celula2 esta PRONTA ha 44 min e nao esta na fila de integracao: trabalho terminado que ninguem vai buscar. Se ela tem portao, o fatia.done tem de DIZER qual; se nao tem, reentregue com bin/entregar.sh..
...
## PENDENTES DO RONALD (59) -- aval, "!", corte e smoke esperando voce

_Gerada de `PENDENTES_RONALD.json` por `bin/gerar_pendentes.py` em 22/09 23:10. Entra quando o DRY/pedido nasce, sai quando aplicado. `pendentes` = 59 (aval 10 - corte 35 - smoke 14); `mais_velho_h` = 247 (esperado: nenhum acima de 24 h -- hoje **39 acima**)._
...
| 7 | smoke | GEO-PAINEL: pino de GPS do painel situacional volta a abrir o mapa (front na arvore, fora do git) | 17/09 13:00 | **130 h** | 1 tela | `smoke OK: GEO-PAINEL` |
| 8 | smoke | COBRAR-DIA: botao 'Cobrar este dia' na caixa do Resolver dia | 17/09 10:47 | **132 h** | 1 tela | `smoke OK: COBRAR-DIA` |
| 9 | smoke | SOLIC-FLAG: Solicitacoes no app por empresa (desligado por padrao) | 17/09 11:34 | **131 h** | 1 tela | `smoke OK: SOLIC-FLAG` |
| 10 | smoke | FORM-CATALOGO: 'Nova solicitacao' com 7 areas pelo catalogo | 17/09 12:24 | **130 h** | 1 tela | `smoke OK: FORM-CATALOGO` |
| 11 | smoke | PROPOSTA-EVIDENCIA: faixa da proposta de escala no fio com a evidencia dos 28 dias | 17/09 17:54 | **125 h** | 1 tela | `smoke OK: PROPOSTA-EVIDENCIA` |
...
| 14 | smoke | TELA-FOLGA-ROTULO: no calendario do colab, o Ok verde so aparece em dia de trabalho com o marco batido; folga, feriado e ausencia nunca ganham Ok (front na arvore, fora do git) | 18/09 17:16 | **101 h** | 1 tela | `smoke OK: TELA-FOLGA-ROTULO` |
| 15 | smoke | AUSENCIA-TIPO-REJULGA-TELA: painel da ausencia de tipo que nao pede documento diz 'Documento: nao exigido para este tipo' no lugar do aviso vermelho (front na arvore, fora do git) | 18/09 19:31 | **99 h** | 1 tela | `smoke OK: AUSENCIA-TIPO-REJULGA-TELA` |
| 16 | smoke | PAUTA-DO-DIA: 'Abrir Pauta DP' no dia de competencia fechada cria a pauta colada no dia (o dia no corpo); a ficha do colab lista as pautas dele (front na arvore, fora do git) | 18/09 19:42 | **99 h** | 1 tela | `smoke OK: PAUTA-DO-DIA` |
| 17 | smoke | CADASTRO-X-REALIDADE: Plano de Escalas > 'Cadastro x realidade' (no lugar de 'Escalas propostas (LIMBO)') -- lista por posto, vinculo, assinatura e botao da acao; coluna destino sugerido; contador 300 de 533; botao PDF padrao com 'o que este PDF contem' ao lado e o PDF da mesma lista (front na arvore, fora do git) | 18/09 20:56 | **98 h** | 1 tela + 1 PDF | `smoke OK: CADASTRO-X-REALIDADE` |
...
| 20 | corte | Tela para segunda: RECUSA-COM-PORTA, UI-TOOLTIP-FILA e WIZARD-12x36-FDS nao tem definicao em ticket nem no relato -- o que e cada uma? | 19/09 15:05 | **80 h** | 3 fatias | `(uma linha por fatia com o que ela faz)` |
